...
if ($uid > 0) {
	...
	if ($rss_share == 'on') {
		...
		$sql = "SELECT cid, comment_timestamp, comment FROM ".$wpdb->base_prefix."cpcommunitie_comments WHERE is_group != 'on' AND comment_parent = 0 AND author_uid = %d AND subject_uid = %d ORDER BY cid DESC LIMIT 0,25";
		...
		foreach ($activities as $activity) {
			...
			echo '<item>';
				...
				echo '<link>'.$profile_url.__cpc__string_query($profile_url).'uid='.$uid.'&amp;post='.$activity->cid.'</link>';
				echo '<guid>'.$profile_url.__cpc__string_query($profile_url).'uid='.$uid.'&amp;post='.$activity->cid.'</guid>';
		...
		echo '<item>';
			echo '<title>'.__('Diese Aktivität ist nicht öffentlich verfügbar.', 'cp-communitie').'</title>';
		echo '</item>';
