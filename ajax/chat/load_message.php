...
if (is_user_logged_in()) {
	...
	$check_unreceived = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix."cpcommunitie_chat2 WHERE from_id=%s AND to_id=%s AND sent < %s AND recd='0' ORDER BY id", $_POST['partner_id'], $_POST['own_id'], $sent_time));
	if($check_unreceived){
		foreach ($check_unreceived as $check_ur_row) {
			...
			$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix."cpcommunitie_chat2 SET recd='1',sent=%s WHERE id=%s AND recd='0'", time(), $check_ur_row->id));
		}
	
		//insert info message as system into current chat
		$wpdb->query($wpdb->prepare("INSERT INTO ".$wpdb->base_prefix."cpcommunitie_chat2 (from_id,to_id,message,sent,system_message) VALUES(%s,%s,'".__('These are unreceived messages from the previous chat session!', CPC_TEXT_DOMAIN)."',%s,'yes')", $_POST['partner_id'], $_POST['own_id'], time()));
	}

	// set typing flag, or clear
	$is_typing = $_POST['typing'];
	if ($is_typing) {
		$sql = "INSERT INTO ".$wpdb->base_prefix."cpcommunitie_chat2_typing (typing_from,typing_to) VALUES (%d,%d)";
		$wpdb->query($wpdb->prepare($sql, $_POST['own_id'], $_POST['partner_id']));
	} else {
		$sql = "DELETE FROM ".$wpdb->base_prefix."cpcommunitie_chat2_typing WHERE typing_from = %d AND typing_to = %d";
		$wpdb->query($wpdb->prepare($sql, $_POST['own_id'], $_POST['partner_id']));
	}

	// get typing flag
	$sql = "SELECT * FROM ".$wpdb->base_prefix."cpcommunitie_chat2_typing WHERE typing_from = %d AND typing_to = %d";
	...
	//load messages
	$res = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix."cpcommunitie_chat2 WHERE (from_id=%s AND to_id=%s AND sent > %s) OR (from_id=%s AND to_id=%s AND sent > %s) ORDER BY sent", $_POST['own_id'], $_POST['partner_id'], $sent_time, $_POST['partner_id'], $_POST['own_id'], $sent_time));
	if($res){
		$last_id = '';
		$from_id = '';
		foreach($res as $row) {
			...
			if($row->to_id == $_POST['own_id']){
				$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix."cpcommunitie_chat2 SET recd='1' WHERE id=%s AND recd='0'", $row->id));
			}		
