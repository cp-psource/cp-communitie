...
if (is_user_logged_in() && isset($_POST['status'])) {
	...
	$result = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix."cpcommunitie_chat2_users WHERE id = %d", $_POST['own_id']));
	if ($result) {
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix."cpcommunitie_chat2_users SET chat_status=%s, offlineshift=%d WHERE id=%d", $chat_status, $offlineshift, $_POST['own_id']));
	} else {						
		$wpdb->query( $wpdb->prepare( "
			INSERT INTO ".$wpdb->base_prefix."cpcommunitie_chat2_users 
			( 	id,
				chat_status, 
				offlineshift
			)
