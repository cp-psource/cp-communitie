...
if ($_POST['action'] == 'acceptAnswer') {
	...
	if (is_user_logged_in()) {
		...
		if (__cpc__safe_param($tid)) {
			
			$r = 'OK';

			// Get parent tid first
			$sql = "SELECT topic_parent, topic_owner FROM ".$wpdb->prefix."cpcommunitie_topics WHERE tid = %d";
			...
			$sql = "SELECT topic_owner FROM ".$wpdb->prefix."cpcommunitie_topics WHERE tid = %d";
			$original_post_owner = $wpdb->get_var($wpdb->prepare($sql, $topic_parent));
			
			// Check to see if any answers already exits
			$sql = "SELECT COUNT(topic_answer) FROM ".$wpdb->prefix."cpcommunitie_topics WHERE topic_parent = %d AND topic_answer = 'on'";
			...
			$sql = "UPDATE ".$wpdb->prefix."cpcommunitie_topics SET topic_answer = '' WHERE topic_parent = %d";
			$wpdb->get_var($wpdb->prepare($sql, $topic_parent));
			
			// Finally update new accepted answer
			$sql = "UPDATE ".$wpdb->prefix."cpcommunitie_topics SET topic_answer = 'on' WHERE tid = %d";
			...
			do_action('cpcommunitie_forum_answer_accepted_hook', $tid, $topic_owner, $topic_parent, $original_post_owner, $current_user->ID);
...
if ($_POST['action'] == 'removeUploadedImage') {
	...
	if (get_option(CPC_OPTIONS_PREFIX.'_img_db') == "on") {
		...
		if ($blog_id > 1) {			
			$src = get_option(CPC_OPTIONS_PREFIX.'_img_path').'/'.$blog_id.'/forum/'.$folder.'/'.$file;
		} else {
			$src = get_option(CPC_OPTIONS_PREFIX.'_img_path').'/forum/'.$folder.'/'.$file;
		}
...
if ($_POST['action'] == 'updateTopicScore') {
	...
	if (is_user_logged_in()) {
		...
		if (__cpc__safe_param($tid)) {
			...
			$sql = "SELECT COUNT(*) FROM ".$wpdb->prefix."cpcommunitie_topics_scores WHERE tid = %d and uid = %d";
			...
			$wpdb->query( $wpdb->prepare( "
				INSERT INTO ".$wpdb->prefix."cpcommunitie_topics_scores 
				( 	tid,
					uid, 
					score, 
					topic_date
			...
			$sql = "SELECT SUM(score) FROM ".$wpdb->prefix."cpcommunitie_topics_scores WHERE tid = %d";
...
if ($_POST['action'] == 'deleteReply') {
	...
	if (is_user_logged_in()) {
		...
		if (can_manage_forum() || $owner == $current_user->ID) {
			if (__cpc__safe_param($tid)) {
				$wpdb->query($wpdb->prepare("DELETE FROM ".$wpdb->prefix."cpcommunitie_topics WHERE tid = %d", $tid));
			}
		
			echo $tid;
			
			// Hook for more actions
			do_action('cpcommunitie_forum_delete_reply_hook', $owner, $tid);			
...
if ($_POST['action'] == 'deleteTopic') {
	...
	if (is_user_logged_in()) {
		...
		$tid = $_POST['topic_id'];
		$topic_owner = $wpdb->get_var($wpdb->prepare("SELECT topic_owner FROM ".$wpdb->prefix."cpcommunitie_topics WHERE tid = %d", $tid));

		if (current_user_can('level_10') || $current_user->ID == $topic_owner) {
			if (__cpc__safe_param($tid)) {
				$wpdb->query($wpdb->prepare("DELETE FROM ".$wpdb->prefix."cpcommunitie_topics WHERE topic_parent = %d", $tid));
				$wpdb->query($wpdb->prepare("DELETE FROM ".$wpdb->prefix."cpcommunitie_topics_scores WHERE tid = %d", $tid));
				$wpdb->query($wpdb->prepare("DELETE FROM ".$wpdb->prefix."cpcommunitie_topics WHERE tid = %d", $tid));
				$wpdb->query($wpdb->prepare("DELETE FROM ".$wpdb->prefix."cpcommunitie_subs WHERE tid = %d", $tid));
			...
			// Hook for more actions
			do_action('cpcommunitie_forum_delete_topic_hook', $topic_owner, $tid);			
...
if ($_POST['action'] == 'forumNewPost') {
	...
	if (is_user_logged_in()) {
		...
		if (get_option(CPC_OPTIONS_PREFIX.'_striptags') == 'on') {
			$new_topic_subject = strip_tags($new_topic_subject);
			$new_topic_text = strip_tags($new_topic_text);
		}
		
		// Check for moderation
		if (get_option(CPC_OPTIONS_PREFIX.'_moderation') == "on" && __cpc__get_current_userlevel() < 5) {
			$topic_approved = "";
		} else {
			$topic_approved = "on";
		}
		...
		$chatroom_banned = get_option(CPC_OPTIONS_PREFIX.'_chatroom_banned');
		if ($chatroom_banned != '') {
		...
		// Create stub		
		$stub = __cpc__create_stub($new_topic_subject);
		...
		if (get_option(CPC_OPTIONS_PREFIX.'_img_db') == "on") {
			...
			$wpdb->query( $wpdb->prepare( "UPDATE ".$wpdb->base_prefix."cpcommunitie_topics_images SET tid = %d WHERE tid = 0 AND uid = %d", $new_tid, $current_user->ID ));
			
		} else {

			if ($blog_id > 1) {
				$tmp_path = get_option(CPC_OPTIONS_PREFIX.'_img_path').'/'.$blog_id.'/forum/0_'.$current_user->ID.'_tmp';
				$to_path = get_option(CPC_OPTIONS_PREFIX.'_img_path').'/'.$blog_id.'/forum/'.$new_tid;
			} else {
				$tmp_path = get_option(CPC_OPTIONS_PREFIX.'_img_path').'/forum/0_'.$current_user->ID.'_tmp';
				$to_path = get_option(CPC_OPTIONS_PREFIX.'_img_path').'/forum/'.$new_tid;
			}
		...
		if ($group_id > 0) {
			$wpdb->query( $wpdb->prepare( "UPDATE ".$wpdb->base_prefix."cpcommunitie_groups SET last_activity = %s WHERE gid = %d", array( date("Y-m-d H:i:s"), $group_id ) ));
		}
		...
		if (get_option(CPC_OPTIONS_PREFIX.'_permalink_structure') && $group_id == 0) {
			...
			$url = __cpc__get_url('forum').'/'.$perma_cat.$stub;
			$cat_url = __cpc__get_url('forum').'/'.$perma_cat;
		} else {
...
if ($_POST['action'] == 'forumNewPostEmails') {
	...
	if (is_user_logged_in()) {
		...
		$sql = "SELECT * FROM ".$wpdb->prefix."cpcommunitie_topics WHERE tid = %d";
		...
		if ($topic->topic_approved == 'on') {
			...
			if ($group_id == 0) {
				...
				$sql = "SELECT user_email, ID
						FROM ".$wpdb->base_prefix."users u RIGHT JOIN ".$wpdb->prefix."cpcommunitie_subs s ON s.uid = u.ID 
						INNER JOIN ".$wpdb->prefix."cpcommunitie_group_members m ON s.uid = m.member_id AND s.cid = m.group_id
						WHERE s.tid = 0 AND u.ID != %d AND s.cid = %d";

				$sql = "SELECT user_email, ID 
						FROM ".$wpdb->base_prefix."users u 
						INNER JOIN ".$wpdb->prefix."cpcommunitie_subs s ON (s.uid = u.ID AND s.cid = %d)
						INNER JOIN ".$wpdb->prefix."cpcommunitie_group_members m ON (s.uid = m.member_id AND m.group_id = %d)
						WHERE s.tid = 0 AND u.ID != %d";

				$cid = $group_id + 10000;
				$query = $wpdb->get_results($wpdb->prepare($sql, $cid, $group_id, $current_user->ID));
			}
			if (CPC_DEBUG) echo $wpdb->last_query.'<br><br>';
			
			// Work out mail subject
			if (strpos(get_option(CPC_OPTIONS_PREFIX.'_subject_forum_new'), '[topic]') !== FALSE) {
				$topic = $wpdb->get_var($wpdb->prepare("SELECT topic_subject FROM ".$wpdb->prefix."cpcommunitie_topics WHERE tid = %d", $new_tid));
				$subject = str_replace("[topic]", $topic, get_option(CPC_OPTIONS_PREFIX.'_subject_forum_new'));
			} else {
				$subject = get_option(CPC_OPTIONS_PREFIX.'_subject_forum_new');
			}	
			...
			if ($query) {					
				...
				foreach ($query as $user) {
					...
					apply_filters ('__cpc__forum_newtopic_filter', $user->ID, $current_user->ID, $current_user->display_name, $url);

					// Add to list of those sent to
					$email_list .= $user->ID.',';
					
					// Send mail
					__cpc__sendmail($user->user_email, $subject.$subject_add, $body_prefix.$body);						
				}						
			}
			...
			if (get_option(CPC_OPTIONS_PREFIX.'_allow_subscribe_all') == "on") {
				...
				if ($group_id == 0) {
					$sql = "SELECT ID,user_email FROM ".$wpdb->base_prefix."users u 
						...
						WHERE u.ID != %d AND
						m.meta_key = 'cpcommunitie_forum_all' AND
						m.meta_value = 'on' AND
					...
					$sql = "SELECT ID,user_email FROM ".$wpdb->base_prefix."users u 
					    LEFT JOIN ".$wpdb->base_prefix."usermeta m ON u.ID = m.user_id
					    INNER JOIN ".$wpdb->prefix."cpcommunitie_group_members gm ON (u.ID = gm.member_id AND gm.group_id = %d)
						WHERE u.ID != %d AND
						m.meta_key = 'cpcommunitie_forum_all' AND
						m.meta_value = 'on' AND
						u.ID NOT IN (".$email_list.")";
				...
				if ($query) {						
					foreach ($query as $user) {	
						...
						if (CPC_DEBUG) echo "Checking ".$user->user_email."<br>";
						$continue = false;
						if ($group_id == 0) {
							
							if (CPC_DEBUG) echo "Not a group<br>";
							...
							if (strpos(strtolower($cat_roles), 'everyone,') !== FALSE || strpos(strtolower($cat_roles), $user_role.',') !== FALSE) {	
								$continue = true;
								if (CPC_DEBUG) "Valid permission<br>";								
							} else {
								if (CPC_DEBUG) "In valid permission<br>";								
							}
							
						} else {						
							$user_email = $user->user_email;	
							if (CPC_DEBUG) echo "A group<br>";
							$continue = true;
						}
						
						if ($continue) {
							
							if (CPC_DEBUG) echo "Continue.<br>";
							
							// Filter to allow further actions to take place
							apply_filters ('__cpc__forum_newtopic_filter', $user->ID, $current_user->ID, $current_user->display_name, $url);
				
							// Send mail
							__cpc__sendmail($user_email, $subject.$subject_add, $body_prefix.$body);							
							
						} else {
							if (CPC_DEBUG) echo "Don't continue.<br>";
						}
...
if ($_POST['action'] == 'getForum') {
	...
	echo __cpc__getForum($cat_id, $limit_from, $group_id);
...
if ($_POST['action'] == 'replycomment') {
	...
	if (is_user_logged_in()) {
		...
		$striptags = get_option(CPC_OPTIONS_PREFIX.'_striptags');
		if ($striptags == 'on') {
		...
		if ($comment_text != '') {
			...
			// Hook for more actions
			do_action('cpcommunitie_forum_replycomment_hook', $current_user->ID, $current_user->display_name, $new_id);			
...
if ($_POST['action'] == 'replycommentemails') {
	...
	if (is_user_logged_in()) {
		...
		$rid = $_POST['rid'];
		$sql = "SELECT topic_parent, topic_post FROM ".$wpdb->prefix."cpcommunitie_topics WHERE tid = %d";
		$comment = $wpdb->get_row($wpdb->prepare($sql, $rid));
		if (CPC_DEBUG) echo 'RID='.$rid.'<br />';
		...
		$parent_tid = $wpdb->get_var($wpdb->prepare("SELECT tid FROM ".$wpdb->prefix."cpcommunitie_topics WHERE tid = %d", $reply->topic_parent));
		$stub = $wpdb->get_var($wpdb->prepare("SELECT stub FROM ".$wpdb->prefix."cpcommunitie_topics WHERE tid = %d", $reply->topic_parent));
		...
		$sql = "SELECT user_email, ID
				FROM ".$wpdb->base_prefix."users u 
				RIGHT JOIN ".$wpdb->prefix."cpcommunitie_subs ON ".$wpdb->prefix."cpcommunitie_subs.uid = u.ID 
				WHERE u.ID != %d AND tid = %d";
		$query = $wpdb->get_results($wpdb->prepare($sql, $current_user->ID, $parent_tid));

		if (CPC_DEBUG) echo 'Checking subscription: '.$wpdb->last_query.'<br />';
			
		if ($query) {						
			foreach ($query as $user) {	

				// Filter to allow further actions to take place
				if (CPC_DEBUG) echo 'Applying __cpc__forum_newreplycomment_filter: '.$user->ID.','.$current_user->ID.','.$current_user->display_name.','.$url.'<br />';
				apply_filters ('__cpc__forum_newreplycomment_filter', $user->ID, $current_user->ID, $current_user->display_name, $url);
		
				// Keep track of who sent to so far
				$email_list .= $user->ID.',';
		
				// Send mail
				if (strpos(get_option(CPC_OPTIONS_PREFIX.'_subject_forum_reply'), '[topic]') !== FALSE) {
					$subject = str_replace("[topic]", $parent, get_option(CPC_OPTIONS_PREFIX.'_subject_forum_reply'));
				} else {
					$subject = get_option(CPC_OPTIONS_PREFIX.'_subject_forum_reply');
				}
				__cpc__sendmail($user->user_email, $subject.$subject_add, $body);							
			}
		}						
		...
		if (CPC_DEBUG) echo 'Checking subscribe-to-all: '.$wpdb->last_query.'<br />';
		...
		if ($query) {						
			foreach ($query as $user) {	
				...
				if ($group_id == 0 || __cpc__member_of($group_id) == "yes") {
					...
					// Check in this topics category level
					if (CPC_DEBUG) echo 'Role check: '.$group_id.','.__cpc__member_of($group_id).','.strtolower($cat_roles).','.$user_role.'<br />';
					if ((__cpc__member_of($group_id) == "yes") || strpos(strtolower($cat_roles), 'everyone,') !== FALSE || strpos(strtolower($cat_roles), $user_role.',') !== FALSE) {	 
						...
						__cpc__sendmail($user_email, $subject.$subject_add, $body);							
...
if ($_POST['action'] == 'reply') {
	...
	if (is_user_logged_in()) {
		...
		$striptags = get_option(CPC_OPTIONS_PREFIX.'_striptags');
		if ($striptags == 'on' && get_option(CPC_OPTIONS_PREFIX.'_use_wysiwyg') != 'on')
		...
		if ($reply_text != '') {
			...
			// Check for moderation
			$moderation = get_option(CPC_OPTIONS_PREFIX.'_moderation');
			if ($moderation == "on") {
				$topic_approved = "";
			} else {
				$topic_approved = "on";
			...
			if (CPC_DEBUG && $show_debug) echo $wpdb->last_query.'<br />';
			...
			if (get_option(CPC_OPTIONS_PREFIX.'_img_db') == "on") {
				...
				$wpdb->query( $wpdb->prepare( "UPDATE ".$wpdb->base_prefix."cpcommunitie_topics_images SET tid = %d WHERE uid = %d AND tid = 0", $new_id, $current_user->ID ));
				
			} else {
				
				// File system
				if ($blog_id > 1) {
					$to_path = get_option(CPC_OPTIONS_PREFIX.'_img_path').'/'.$blog_id.'/forum/'.$tid.'/'.$new_id;
					$tmp_path = get_option(CPC_OPTIONS_PREFIX.'_img_path').'/'.$blog_id.'/forum/'.$tid.'_'.$current_user->ID.'_tmp';
				} else {
					$to_path = get_option(CPC_OPTIONS_PREFIX.'_img_path').'/forum/'.$tid.'/'.$new_id;
					$tmp_path = get_option(CPC_OPTIONS_PREFIX.'_img_path').'/forum/'.$tid.'_'.$current_user->ID.'_tmp';
				}
				if (CPC_DEBUG && $show_debug) echo 'Looking for images in '.$tmp_path.' ';
				if (file_exists($tmp_path)) {
					if (CPC_DEBUG && $show_debug) echo 'FILE EXISTS: '.$tmp_path.' ';
					mkdir($to_path, 0777, true);
					// copy tmp files to new location
					$handler = opendir($tmp_path);
					while ($file = readdir($handler)) {
						if ($file != "." && $file != ".." && $file != ".DS_Store") {
							if (CPC_DEBUG && $show_debug) echo 'Copy '.$tmp_path.'/'.$file.' to '.$to_path.'/'.$file.'<br />';
							copy($tmp_path.'/'.$file, $to_path.'/'.$file);
							unlink($tmp_path.'/'.$file);
						}
					}
					...
					if (CPC_DEBUG && $show_debug) echo 'FILE DOES NOT EXIST: '.$tmp_path.'<br />';
				}
			...
			$bump_topics = get_option(CPC_OPTIONS_PREFIX.'_bump_topics');
			if ($bump_topics == 'on') {
				$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix."cpcommunitie_topics SET topic_date = '".date("Y-m-d H:i:s")."' WHERE tid = %d", $tid) );					
			}

			// Add new forum reply to activity
			$has_plus_activated = false;
			if (get_option(CPC_OPTIONS_PREFIX.'__cpc__profile_plus_activated') || get_option(CPC_OPTIONS_PREFIX.'__cpc__profile_plus_network_activated')) $has_plus_activated = true;

			$option_to_check = ($group_id == 0) ? CPC_OPTIONS_PREFIX.'_show_forum_replies_on_activity' : CPC_OPTIONS_PREFIX.'_show_group_replies_on_activity';
			if ($has_plus_activated && get_option($option_to_check) && !get_option(CPC_OPTIONS_PREFIX.'_cpc_lite')) {
				if (get_option(CPC_OPTIONS_PREFIX.'_permalink_structure') && $group_id == 0) {
					$stub = $wpdb->get_var($wpdb->prepare("SELECT stub FROM ".$wpdb->prefix."cpcommunitie_topics WHERE tid = %d", $tid));
					$perma_cat = __cpc__get_forum_category_part_url($cat_id);
					$url = $forum_url.'/'.$perma_cat.$stub;							
		...
		if ($topic_approved == "on") {
			...
			if ($query) {						
				foreach ($query as $user) {	
					...
					// Filter to allow further actions to take place
					if (CPC_DEBUG) echo 'Applying __cpc__forum_newreply_filter: '.$user->ID.','.$current_user->ID.','.$current_user->display_name.','.$url.'<br />';
...
if ($_POST['action'] == 'getActivity') {
	...
	$grouppage = __cpc__get_url('group');
	...
	$html = '<div id="forum_activity_div">';
		...
		$html .= '<div id="forum_activity_all_new_topics">';
			...
			if ($topics) {
				foreach ($topics as $topic) {
					...
					if ($topic->topic_group == 0 || __cpc__member_of($topic->topic_group) == 'yes') {
						
						$html .= "<div class='forum_activity_new_topic_subject'>";
						if ($topic->topic_group == 0) {

							if (get_option(CPC_OPTIONS_PREFIX.'_permalink_structure') && $group_id == 0) {
								$stub = $wpdb->get_var($wpdb->prepare("SELECT stub FROM ".$wpdb->prefix."cpcommunitie_topics WHERE tid = %d", $topic->tid));
								$perma_cat = __cpc__get_forum_category_part_url($topic->topic_category);
								$url = $thispage.'/'.$perma_cat.$stub;							
								$html .= "<a href='".$url."'>".__cpc__bbcode_remove(stripslashes($topic->topic_subject))."</a> ";
							} else {
								$html .= "<a href='".$thispage.$q.'cid='.$topic->topic_category.'&show='.$topic->tid."'>".__cpc__bbcode_remove(stripslashes($topic->topic_subject))."</a>";
							}
							
						} else {
							$html .= "<a href='".$grouppage.$q.'gid='.$topic->topic_group.'&cid='.$topic->topic_category.'&show='.$topic->tid."'>".__cpc__bbcode_remove(stripslashes($topic->topic_subject))."</a>";
						}
						$html .= "</div>";
						
						$text = __cpc__bbcode_remove(strip_tags(stripslashes($topic->topic_post)));
				...
				foreach ($topics as $topic) {		
					...
					if ($topic->topic_group == 0 || __cpc__member_of($topic->topic_group) == 'yes') {
						...
						if ($topic->topic_group == 0) {
							...
							$html .= __cpc__time_ago($topic->topic_date);
						} else {
							$html .= "<a href='".$grouppage.$q.'gid='.$topic->topic_group.'&cid='.$topic->topic_category.'&show='.$topic->tid."'>".__cpc__bbcode_remove(stripslashes($topic->topic_subject))."</a>, ".__cpc__time_ago($topic->topic_date);
						}
						$html .= "</div>";
						...
						// Replies
						$replies = $wpdb->get_results($wpdb->prepare("SELECT t.*, u.display_name FROM ".$wpdb->prefix."cpcommunitie_topics t LEFT JOIN ".$wpdb->base_prefix."users u ON t.topic_owner = u.ID WHERE topic_parent = %d ORDER BY tid DESC", $topic->tid));
						if ($replies) {
							$cnt = 0;
							$dt = '';
							$display_name = '';
		...
		$html .= '<div id="forum_activity_replies">';
			...
			LEFT JOIN ".$wpdb->prefix."cpcommunitie_topics t2 ON t.topic_parent = t2.tid 
					...
					if ($topic->topic_group == 0 || __cpc__member_of($topic->topic_group) == 'yes') {
						...
						if (strpos($shown, $topic->topic_parent.",") === FALSE) { 
							...
							if ($topic->topic_group == 0) {
								...
								if (get_option(CPC_OPTIONS_PREFIX.'_permalink_structure') && $group_id == 0) {
									$stub = $wpdb->get_var($wpdb->prepare("SELECT stub FROM ".$wpdb->prefix."cpcommunitie_topics WHERE tid = %d", $topic->topic_parent));
									$perma_cat = __cpc__get_forum_category_part_url($topic->topic_category);
									$url = $thispage.'/'.$perma_cat.$stub;							
									$html .= "<a href='".$url."'>".__cpc__bbcode_remove(stripslashes($topic->topic_subject))."</a> ";
								} else {
									$html .= "<a href='".$thispage.$q.'cid='.$topic->topic_category.'&show='.$topic->topic_parent."'>".__cpc__bbcode_remove(stripslashes($topic->topic_subject))."</a>";
								}	
															
							} else {
								$html .= "<a href='".$grouppage.$q.'gid='.$topic->topic_group.'&cid='.$topic->topic_category.'&show='.$topic->topic_parent."'>".__cpc__bbcode_remove(stripslashes($topic->topic_subject))."</a>";
							}
							$html .= "</div>";	
		...
		if ($posts) {
			...
			foreach ($posts as $post)
			{
				...
				if ($gid > 0 || strpos(strtolower($cat_roles), 'everyone,') !== FALSE || strpos(strtolower($cat_roles), $user_role.',') !== FALSE) {				
					...
					$html .= "<div class='__cpc__forum_activity_row'>";
						$html .= "<div class='__cpc__forum_activity_row_avatar'>";
							$html .= get_avatar($post->topic_owner, 20);
						$html .= "</div>";
						$html .= "<div class='__cpc__forum_activity_row_text'>";
							if ($post->topic_parent > 0) {
								...
								$html .= __cpc__time_ago($post->topic_date).".<br>";

							}
						$html .= "</div>";
						if ($post->topic_date > $cpcommunitie_last_login && $post->topic_owner != $current_user->ID && is_user_logged_in() && get_option(CPC_OPTIONS_PREFIX.'_forum_stars')) {
...
if ($_POST['action'] == 'getThreadsActivity') {
	...
	$html = '<div id="forum_activity_div">';
	$html .= showThreadChildren(0, 0, $gid, $cpcommunitie_last_login);	
	$html .= '</div>';
...
function showThreadChildren($parent, $level, $gid, $cpcommunitie_last_login) {
	...
	if ($posts) {
		...
		foreach ($posts as $post)
		{
			...
			if ($gid > 0 || strpos(strtolower($cat_roles), 'everyone,') !== FALSE || strpos(strtolower($cat_roles), $user_role.',') !== FALSE) {		
				...
				$html .= "<div class='__cpc__forum_activity_row' style='padding-left: ".($level*40)."px; margin-top:".$margin_top."px;min-height:".$avatar_size."px;'>";		
					$html .= "<div class='__cpc__forum_activity_row_avatar' style='padding-left: ".($level*40)."px;'>";
						$html .= get_avatar($post->topic_owner, $avatar_size);
					$html .= "</div>";
					$move_over = ($level == 0) ? 40 : 30;
					$html .= "<div class='__cpc__forum_activity_row_text' style='margin-left: ".$move_over."px;'>";
						if ($post->topic_parent > 0) {
							$text = strip_tags(stripslashes($post->topic_post));
							if ( strlen($text) > $preview ) { $text = substr($text, 0, $preview)."..."; }
							$html .= __cpc__profile_link($post->topic_owner)." ".__('replied', 'cp-communitie')." ";
				...
				$html .= showThreadChildren($post->tid, $level+1, $gid, $cpcommunitie_last_login);
			}			
		...
		if ($favs) {
			foreach ($favs as $fav) {
				...
				if ($fav != '') {
					...
					$post = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix."cpcommunitie_topics WHERE tid = %d", $fav));
					...
					if ($post) {
						...
						$html .= '<div id="fav_'.$fav.'" class="fav_row" style="padding:6px; margin-bottom:10px;">';
							...
							$html .= " <a title='".$fav."' class='__cpc__delete_fav' style='cursor:pointer'><img src='".get_option(CPC_OPTIONS_PREFIX.'_images')."/delete.png' style='width:16px;height:16px' /></a>";
							...
							$replies = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->prefix.'cpcommunitie_topics'." WHERE topic_parent = %d ORDER BY topic_date DESC", $post->tid));
							if ($replies) {
								$cnt = 0;
								$dt = '';
								foreach ($replies as $reply) {
...
if ($_POST['action'] == 'toggleFav') {
	...
	if (is_user_logged_in()) {
		...
		__cpc__update_meta($current_user->ID, "forum_favs", $favs);
...
if ($_POST['action'] == 'getEditDetails') {
	...
	if (is_user_logged_in()) {
		...
		$details = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix.'cpcommunitie_topics'." WHERE tid = %d", $tid)); 
		if ($details->topic_subject == '') {
			$parent = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix.'cpcommunitie_topics'." WHERE tid = %d", $details->topic_parent)); 
		...
		if (!get_option(CPC_OPTIONS_PREFIX.'_use_wysiwyg')) {
...
if ($_POST['action'] == 'updateDigest') {
	...
	// Update meta record exists for user
	__cpc__update_meta($current_user->ID, "forum_digest", "'".$value."'");
	echo $value;
	exit;
...
if ($_POST['action'] == 'updateEditDetails') {
	...
	if (is_user_logged_in()) {
		...
		if ($topic_category == "") {
			$topic_category = $wpdb->get_var($wpdb->prepare("SELECT topic_category FROM ".$wpdb->prefix.'cpcommunitie_topics'." WHERE tid = %d", $tid));
		}

		$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->prefix."cpcommunitie_topics SET topic_category = %d WHERE topic_parent = %d", $topic_category, $tid) );

		$sql = "UPDATE ".$wpdb->prefix."cpcommunitie_topics SET topic_subject = %s, topic_post = %s, topic_category = %d WHERE tid = %d";
		$wpdb->query( $wpdb->prepare($sql, $topic_subject, $topic_post, $topic_category, $tid) );
			
		$parent = $wpdb->get_var($wpdb->prepare("SELECT topic_parent FROM ".$wpdb->prefix.'cpcommunitie_topics'." WHERE tid = %d", $tid));
...
// AJAX function to subscribe/unsubscribe to cpcommunitie topic
if ($_POST['action'] == 'updateForum') {
	...
	if (is_user_logged_in()) {
		...
		// Store subscription if wanted
		if (__cpc__safe_param($tid)) {
			$wpdb->query($wpdb->prepare("DELETE FROM ".$wpdb->prefix."cpcommunitie_subs WHERE uid = %d AND tid = %d", $current_user->ID, $tid));
		}
...
if ($_POST['action'] == 'updateForumSticky') {
	...
	if (is_user_logged_in()) {
		...
		// Store subscription if wanted
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."cpcommunitie_topics SET topic_sticky = %d WHERE tid = %d", $value, $tid));
...
if ($_POST['action'] == 'updateTopicReplies') {
	...
	if (is_user_logged_in()) {
		...
		// Store subscription if wanted
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."cpcommunitie_topics SET allow_replies = %s WHERE tid = %d", $value, $tid));
...
if ($_POST['action'] == 'toggleForInfo') {
	...
	if (is_user_logged_in()) {
		...
		$topics = $wpdb->prefix . 'cpcommunitie_topics';
...
// AJAX function to subscribe/unsubscribe to new cpcommunitie topics
if ($_POST['action'] == 'updateForumSubscribe') {

	if (is_user_logged_in()) {

		$subs = $wpdb->prefix . 'cpcommunitie_subs';

		$action = $_POST['value'];
		$cid = $_POST['cid'];

		// Store subscription if wanted
		if (__cpc__safe_param($cid))
			$wpdb->query($wpdb->prepare("DELETE FROM ".$wpdb->prefix."cpcommunitie_subs WHERE uid = %d AND tid = 0 AND (cid = %d OR cid = 0)", $current_user->ID, $cid));
...
if ($_POST['action'] == 'socialShare') {
	if (is_user_logged_in()) {
		do_action('cpcommunitie_forum_socialmedia_hook', $current_user->ID, isset($_POST['destination']) ? $_POST['destination'] : 'error');			
	}
	exit;
}
			...
			foreach ($topics as $topic) {	
				...
				if ($topic->topic_group > 0 || strpos(strtolower($cat_roles), 'everyone,') !== FALSE || strpos(strtolower($cat_roles), $user_role.',') !== FALSE) {					
					...
					if ($gid == 0 || __cpc__member_of($gid) == "yes") {
						...
						$html .= "<div class='__cpc__search_subject_row_div'>";
						
							$html .= "<div class='__cpc__search_subject_div'>";
							...
							if ($topic->topic_parent != 0) {
								...
								$topic_subject = preg_replace(
								  "/(>|^)([^<]+)(?=<|$)/esx",
								  "'\\1' . str_replace('" . $term . "', '<span class=\"__cpc__search_highlight\">" . $term . "</span>', '\\2')",
								  $topic_subject
								);

								if (get_option(CPC_OPTIONS_PREFIX.'_permalink_structure') && $group_id == 0) {
									$stub = $wpdb->get_var($wpdb->prepare("SELECT stub FROM ".$wpdb->prefix."cpcommunitie_topics WHERE tid = %d", $topic->parent_tid));
									$perma_cat = __cpc__get_forum_category_part_url($topic->topic_category);
									$url = $thispage.$perma_cat.$stub;							
									$html .= "<a class='__cpc__search_subject' href='".$url."'>".stripslashes($topic_subject)."</a> ";
								} else {
							...
							for ($i = 0; $i <= strlen($text)-strlen($term); $i++) {
								if ( substr(strtolower($text), $i, strlen($term)) == strtolower($term) ) {
									...
									$snippet = preg_replace('/('.$term.')/i', "<span class=\"__cpc__search_highlight\">$1</span>", $snippet); 
									$result .= $snippet;
								}
							}
