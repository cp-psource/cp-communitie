...
if (isset($_GET['term'])) {
	...
	$sql = "SELECT * FROM ".$wpdb->prefix."cpcommunitie_groups WHERE  
	( name LIKE '%".$term."%') OR 
	...
	if ($list) {
		foreach ($list as $item) {
			...
			$name = $item->name != '' ? stripslashes($item->name) : __('[No name]', CPC_TEXT_DOMAIN);
			$row_array['name'] = $name;
...
if (isset($_POST['action']) && $_POST['action'] == 'createGroup') {
	...
	if (is_user_logged_in()) {
		...
		$wpdb->query( $wpdb->prepare( "
			INSERT INTO ".$wpdb->prefix."cpcommunitie_groups
			( 	name, 
				description,
				last_activity,
				private,
...
if (isset($_POST['group_id']) && $_POST['group_id'] != '') {
	header("Location: ".__cpc__get_url('group')."?gid=".$_POST['group_id']);
	exit;
}
if (isset($_POST['group']) && $_POST['group'] != '') {
	header("Location: ".__cpc__get_url('groups')."?term=".$_POST['group']);
	exit;
}
...
if ($_POST['action'] == 'getGroups') {
	...
	$url = get_option(CPC_OPTIONS_PREFIX.'_group_url');
	...
	if ($groups) {
		...
		$html = __("No groups created yet....", CPC_TEXT_DOMAIN);
	}
...
if ($_POST['action'] == 'get_user_list') {
	...
	$sql = "SELECT DISTINCT u.*, m.admin FROM ".$wpdb->base_prefix."users u
			...
			AND u.ID NOT IN (SELECT member_id FROM ".$wpdb->prefix."cpcommunitie_group_members WHERE group_id = %d)
	...
	if ($users) {
		foreach ($users as $user) {
			if ($user->admin != 'on') {
				...
				$html .= "<img src='".get_option(CPC_OPTIONS_PREFIX.'_images')."/add.png' style='width:12px; height:12px; float:right; margin-left:3px;' alt='add' /> ";
...
if ($_POST['action'] == 'get_group_members') {
	...
	$sql = "DELETE FROM ".$wpdb->prefix."cpcommunitie_group_members WHERE member_id = 0";
	$wpdb->query($sql);

	// Now get the members
	$sql = "SELECT u.ID, u.display_name, u.user_login FROM ".$wpdb->prefix."cpcommunitie_group_members m
	...
	if ($users) {
		foreach ($users as $user) {
			$html .= '<div class="user_list_item" id="'.$user->ID.'" style="clear:both; cursor:pointer; float:left; border:1px solid #aaa; background-color:#ccc;margin-bottom:2px;padding:2px;margin-right:2px;">';
			$html .= "<img src='".get_option(CPC_OPTIONS_PREFIX.'_images')."/cross.png' style='width:12px; height:12px; float:right; margin-left:3px;' alt='add' /> ";
			$html .= $user->display_name.' ('.$user->ID.')';
			$html .= '</div>';
		}	
	}
...
if ($_POST['action'] == 'add_group_members') {
	...
	$sql = "DELETE FROM ".$wpdb->prefix."cpcommunitie_group_members WHERE group_id = %d AND admin != 'on'";
	...
	foreach ($ids as $id) {
		...
		if ($id > 0) {
			...
			$insert = $wpdb->insert( 
				$wpdb->prefix."cpcommunitie_group_members", 
				array( 
					'group_id' => $gid, 
					'member_id' => $id,
					'admin' => '',
