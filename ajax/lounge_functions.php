...
if ($_POST['action'] == 'get_comments') {
	...
	$max_items = get_option(CPC_OPTIONS_PREFIX."_lounge_max_rows")+0;
	...
	$sql = "SELECT l.lid, l.comment, l.author, l.added, u.display_name, u.ID
		FROM ".$wpdb->base_prefix."cpcommunitie_lounge l 
	...
	foreach ($comment_list as $comment) {
		...
		$add = array (	
			...
			'ID' => $comment->ID,
			'last_activity' => __cpc__get_meta($comment->ID, 'last_activity'),
			'status' => __cpc__get_meta($comment->ID, 'status')
		);
		
		array_push($comment_array, $add);		
	}
	$comments = __cpc__sub_val_sort($comment_array, 'lid', false);
	...
	if ($comments) {
		foreach ($comments as $comment) {
			...
			if ($comment['last_activity'] && $comment['status'] != 'offline') {
				$last_active_minutes = __cpc__convert_datetime($comment['last_activity']);
			...
			$row_array['added'] = __cpc__time_ago($comment['added']);
...
if ($_POST['action'] == 'delete_comment') {
	...
	if ( __cpc__get_current_userlevel($current_user->ID) == 5 ) {
		$sql = "DELETE FROM ".$wpdb->base_prefix."cpcommunitie_lounge WHERE lid = %d";
