...
if ($_POST['action'] == 'bulk_readall') {
	...
	if (is_user_logged_in()) {
		...
		$sql = "UPDATE ".$wpdb->base_prefix."cpcommunitie_mail SET mail_read = 'on' WHERE mail_to = %d";
...
if ($_POST['action'] == 'bulk_recover') {
	...
	if (is_user_logged_in()) {
		...
		if ($tray == 'in') {
			$sql = "UPDATE ".$wpdb->base_prefix."cpcommunitie_mail SET mail_in_deleted = '' WHERE mail_to = %d";
		} else {
			$sql = "UPDATE ".$wpdb->base_prefix."cpcommunitie_mail SET mail_sent_deleted = '' WHERE mail_from = %d";
		}
		$wpdb->query($wpdb->prepare($sql, $current_user->ID));
	...
	if (is_user_logged_in()) {
		...
		if ($scope == 'marked' && $list) {
			...
			foreach($list as $item) {
				if ($tray == 'in') {
					$sql = "UPDATE ".$wpdb->base_prefix."cpcommunitie_mail SET mail_in_deleted = 'on' WHERE mail_mid = %d";
				} else {
					$sql = "UPDATE ".$wpdb->base_prefix."cpcommunitie_mail SET mail_sent_deleted = 'on' WHERE mail_mid = %d";
				}
		...
		if ($scope == 'all') {
				...
				if ($tray == 'in') {
					$sql = "UPDATE ".$wpdb->base_prefix."cpcommunitie_mail SET mail_in_deleted = 'on' WHERE mail_to = %d";
				} else {
					$sql = "UPDATE ".$wpdb->base_prefix."cpcommunitie_mail SET mail_sent_deleted = 'on' WHERE mail_from = %d";
				}
...
if ($_POST['action'] == 'getRecipientId') {
	...
	if (is_user_logged_in()) {
		...
		$sql = "SELECT u.ID FROM ".$wpdb->base_prefix."users u 
			INNER JOIN ".$wpdb->base_prefix."cpcommunitie_friends f ON u.ID = f.friend_to 
...
if ($_POST['action'] == 'getReply') {
	...
	if (is_user_logged_in()) {
		...
		$sql = "SELECT m.*, u.display_name FROM ".$wpdb->base_prefix."cpcommunitie_mail m LEFT JOIN ".$wpdb->base_prefix."users u ON m.mail_from = u.ID WHERE mail_mid = %d";
...
if ($_POST['action'] == 'sendMail') {
	...
	if (is_user_logged_in()) {
		...
		if (!$recipient) {
			...
			// Send mail
			if ( $rows_affected = $wpdb->insert( $wpdb->base_prefix . "cpcommunitie_mail", array( 
			...
			apply_filters ('__cpc__sendmessage_filter', $recipient->ID, $current_user->ID, $current_user->display_name, $mail_id);
		
			// Send real email if chosen
			if ( __cpc__get_meta($recipient->ID, 'notify_new_messages') ) {
				...
				$mail_subject = get_option(CPC_OPTIONS_PREFIX.'_subject_mail_new');
...
if ($_POST['action'] == 'getBox') {
	...
	if (is_user_logged_in()) {
		...
		if ($tray == "in") {
			$sql = "SELECT m.*, u.display_name FROM ".$wpdb->base_prefix."cpcommunitie_mail m LEFT JOIN ".$wpdb->base_prefix."users u ON m.mail_from = u.ID WHERE mail_in_deleted != 'on' ".$unread." AND mail_to = %d AND (u.display_name LIKE '%%%s%%' OR mail_subject LIKE '%%%s%%' OR mail_message LIKE '%%%s%%') ORDER BY mail_mid DESC LIMIT %d, %d";
		} else {
			$sql = "SELECT m.*, u.display_name FROM ".$wpdb->base_prefix."cpcommunitie_mail m LEFT JOIN ".$wpdb->base_prefix."users u ON m.mail_to = u.ID WHERE mail_sent_deleted != 'on' ".$unread." AND mail_from = %d AND (u.display_name LIKE '%%%s%%' OR mail_subject LIKE '%%%s%%' OR mail_message LIKE '%%%s%%') ORDER BY mail_mid DESC LIMIT %d, %d";
		}
		...
		if ($mail) {
			foreach ($mail as $item)
			{
				...
				$row_array['mail_sent'] = __cpc__time_ago($item->mail_sent);
				if ($tray == "in") {
					$row_array['mail_from'] = stripslashes(__cpc__profile_link($item->mail_from));
				} else {
					$row_array['mail_from'] = stripslashes(__cpc__profile_link($item->mail_to));
				}
				if ($item->mail_read != 'on') {
					$row_array['mail_from'] = '<img src="'.get_option(CPC_OPTIONS_PREFIX.'_images').'/new.gif" /> '.$row_array['mail_from'];
				}
				$row_array['mail_subject'] = __cpc__bbcode_remove(stripslashes($item->mail_subject));
				...
				$row_array['message'] = stripslashes(__cpc__bbcode_remove($message));
...
if ($_POST['action'] == 'getMailMessage') {
	...
	if (is_user_logged_in()) {
		...
		if ($tray == "in") {
			$mail = $wpdb->get_row($wpdb->prepare("SELECT m.*, u.display_name FROM ".$wpdb->base_prefix."cpcommunitie_mail m LEFT JOIN ".$wpdb->base_prefix."users u ON m.mail_from = u.ID WHERE mail_mid = %d", $mail_mid));
		} else {
			$mail = $wpdb->get_row($wpdb->prepare("SELECT m.*, u.display_name FROM ".$wpdb->base_prefix."cpcommunitie_mail m LEFT JOIN ".$wpdb->base_prefix."users u ON m.mail_to = u.ID WHERE mail_mid = %d", $mail_mid));
		}
		
		// check that permission is okay
		if ( ($tray == "in" && $mail->mail_to == $current_user->ID) || ($tray != "in" && $mail->mail_from == $current_user->ID) || (__cpc__get_current_userlevel() == 5) ) {
						
			// Swap codes from template
			$msg = stripslashes(str_replace('[]', '', get_option(CPC_OPTIONS_PREFIX.'_template_mail_message')));
			...
			$msg = str_replace("[mail_subject]", "<span style='font-weight:bold'>".stripslashes(__cpc__bbcode_replace($mail->mail_subject))."</span>", $msg);						
			...
			// Sent
			$msg = str_replace("[mail_sent]", __cpc__time_ago($mail->mail_sent), $msg);
	
			// Delete button
			$msg = str_replace("[delete_button]", '<img src="'.get_option(CPC_OPTIONS_PREFIX.'_images').'/delete2.png" id='.$mail_mid.' class="message_delete" />', $msg);
			...
			// Message
			$msg = str_replace("[message]", stripslashes(__cpc__bbcode_replace($mail->mail_message)), $msg);
			
			// Emoticons
			$msg = __cpc__buffer($msg);
			...
			// Mark as read
			if ($tray == "in") {
				$wpdb->query( $wpdb->prepare("UPDATE ".$wpdb->base_prefix."cpcommunitie_mail SET mail_read = 'on' WHERE mail_mid = %d AND mail_to = %d", $mail_mid, $current_user->ID) );
			}
	
			// Fetch new unread count
			$unread = "?!";
			if ($tray == "in") {
				$unread = $wpdb->get_var("SELECT COUNT(*) FROM ".$wpdb->base_prefix.'cpcommunitie_mail'." WHERE mail_to = ".$mail->mail_to." AND mail_".$tray."_deleted != 'on' AND mail_read != 'on'");
			} else {
				$unread = $wpdb->get_var("SELECT COUNT(*) FROM ".$wpdb->base_prefix.'cpcommunitie_mail'." WHERE mail_from = ".$mail->mail_from." AND mail_".$tray."_deleted != 'on' AND mail_read != 'on'");
			}
