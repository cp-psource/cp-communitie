...
// Site search (autocomplete) for small quick search [cpcommunitie-search]
if (isset($_GET['term'])) {
	...
	$groups_sql = get_option(CPC_OPTIONS_PREFIX.'_site_search_groups') ? "UNION ALL SELECT g.gid AS ID, g.name AS display_name, 'group' AS type, g.private AS sharing, '' AS owner, '' AS url FROM ".$wpdb->base_prefix."cpcommunitie_groups g WHERE ( name LIKE '%".$term."%') LIMIT 0,25 " : '';
	$gallery_sql = get_option(CPC_OPTIONS_PREFIX.'_site_search_gallery') ? "UNION ALL SELECT ga.gid AS ID, ga.name AS display_name, 'gallery' AS type, ga.sharing, ga.owner, '' AS url FROM ".$wpdb->prefix."cpcommunitie_gallery ga WHERE ( name LIKE '%".$term."%') LIMIT 0,25 " : '';
	$posts_sql = get_option(CPC_OPTIONS_PREFIX.'_site_search_posts') ? "UNION ALL SELECT p.id AS ID, p.post_title AS display_name, 'post' AS type, 'public' AS sharing, p.post_author AS owner, guid AS url FROM ".$wpdb->prefix."posts p WHERE ( p.post_type = 'post' AND p.post_status = 'publish' AND ( p.post_title LIKE '%".$term."%' OR p.post_content LIKE '%".$term."%' ) ) LIMIT 0,25 " : '';
	$pages_sql = get_option(CPC_OPTIONS_PREFIX.'_site_search_pages') ? "UNION ALL SELECT pa.id AS ID, pa.post_title AS display_name, 'page' AS type, 'public' AS sharing, pa.post_author AS owner, id AS url FROM ".$wpdb->prefix."posts pa WHERE ( pa.post_type = 'page' AND pa.post_status = 'publish' AND ( pa.post_title LIKE '%".$term."%' OR pa.post_content LIKE '%".$term."%' ) ) LIMIT 0,25 " : '';
	$topics_sql = get_option(CPC_OPTIONS_PREFIX.'_site_search_topics') ? "UNION ALL SELECT t.tid AS ID, t.topic_post AS display_name, 'topic' AS type, 'public' AS sharing, t.topic_owner AS owner, topic_parent AS url FROM ".$wpdb->prefix."cpcommunitie_topics t WHERE ( t.topic_subject LIKE '%".$term."%' OR t.topic_post LIKE '%".$term."%' ) AND t.topic_group = 0 LIMIT 0,25 " : '';
	$sql = "SELECT * FROM (
				...
				switch($item->type) {
					case 'topic': 
						if ( ($item->owner == $current_user->ID) || (strtolower($item->sharing) == 'public') || (is_user_logged_in() && strtolower($item->sharing) == 'everyone') || (strtolower($item->sharing) == 'public') || (strtolower($item->sharing) == 'friends only' && __cpc__friend_of($item->owner, $current_user->ID)) || __cpc__get_current_userlevel() == 5) {
							...
							if (CPC_DEBUG) $row_array['name'] .= ' '.$topic_parent.':'.$topic_grandparent.':'.$row_array['id'];
					        array_push($return_arr,$row_array);
						}
						break;
					case 'post': 
		...
		if ($_POST['action'] == 'menu_plus') {
			...
			foreach ($friends_list as $friend) {
				...
				$add = array (	
					...
					'uid' => $friend->uid,				
					'last_activity' => __cpc__get_meta($friend->following, 'last_activity')
				);
			...
			foreach ($friends_list as $friend) {
				...
				$add = array (	
					'following' => $friend->following,
					'uid' => $friend->uid,				
					'last_activity' => __cpc__get_meta($friend->uid, 'last_activity')
				);
...
if ($_POST['action'] == 'toggle_following') {
	...
	if (is_user_logged_in()) {
		...
		$sql = "SELECT fid FROM ".$wpdb->base_prefix."cpcommunitie_following WHERE uid=%d AND following=%d";
		...
		if ($fid) {
			...
			INSERT INTO ".$wpdb->base_prefix."cpcommunitie_following
			( 	uid, 
				following,
				created
			)
...
if ($_POST['action'] == 'likeDislike') {
	...
	if (is_user_logged_in()) {
		...
		$sql = "DELETE FROM ".$wpdb->base_prefix."cpcommunitie_likes WHERE cid=%d AND uid=%d";
		$wpdb->query($wpdb->prepare($sql, $cid, $current_user->ID));

		$wpdb->query( $wpdb->prepare( "
		INSERT INTO ".$wpdb->base_prefix."cpcommunitie_likes
		( 	type,
			cid, 
			uid,
			liked_date
		...
		$sql = "SELECT author_uid, comment, type, comment_parent FROM ".$wpdb->base_prefix."cpcommunitie_comments WHERE cid = %d";
		$author = $wpdb->get_row($wpdb->prepare($sql, $cid));

		// Support for CubePoints
		do_action('cpcommunitie_profile_like_hook', $author->author_uid, $choice);
		
		if (__cpc__get_meta($author->author_uid, 'notify_likes')) {
...
if ($_POST['action'] == 'getLikesDislikes') {
	...
	if (is_user_logged_in()) {
		...
		$sql = "SELECT l.*, u.display_name FROM ".$wpdb->base_prefix."cpcommunitie_likes l 
		...
		if ($getlikes) {
			...
			foreach ($getlikes as $alike) {
				...
				$link = __cpc__profile_link($alike->uid);
