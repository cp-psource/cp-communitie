...
class cpc_forum {
	...
	function get_categories($top_level='') {
		...
		global $wpdb;
		$sql = "SELECT *
			FROM ".$wpdb->prefix."cpcommunitie_cats
			WHERE cat_parent = %d
			ORDER BY listorder";
	...
	function get_topics($cid='', $start=0, $limit=9999, $order='DESC', $gid=0) {
		...
		if ($cid !== '') {
			...
			$sql = "SELECT t.*, u.display_name,
			    (SELECT count(*) FROM ".$wpdb->prefix."cpcommunitie_topics st WHERE topic_parent = t.tid) AS topic_replies
	...
	function get_topics_count($cid='') {
		...
		if ($cid !== '') {
			...
			$sql = "SELECT count(*) FROM ".$wpdb->prefix."cpcommunitie_topics WHERE topic_category = %d AND topic_parent = 0";
			return $wpdb->get_var($wpdb->prepare($sql, $cid));

		} else {

			$sql = "SELECT count(*) FROM ".$wpdb->prefix."cpcommunitie_topics";
			return $wpdb->get_var($wpdb->prepare($sql, $cid));
		}
	...
	function delete_post($reply_id) {
		...
		$reply = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix."cpcommunitie_topics WHERE tid = %d", $reply_id));
		if ($reply) {
			if ($reply->topic_owner == $current_user->ID || current_user_can('manage_options')) {
				...
				if ( (get_option(CPC_OPTIONS_PREFIX.'_forum_lock')==0) || ($reply->topic_started > $now) || (current_user_can('level_10')) ) {
					// First delete any replies
					$wpdb->query($wpdb->prepare("DELETE FROM ".$wpdb->prefix."cpcommunitie_topics WHERE topic_parent = %d", $reply_id));
					$wpdb->query($wpdb->prepare("DELETE FROM ".$wpdb->prefix."cpcommunitie_topics WHERE tid = %d", $reply_id));
					return $reply_id;					
				} else {
					return false;
				}
	...
	function is_topic_locked($tid) {
		global $wpdb;
		$now = date('Y-m-d H:i:s', time() - get_option(CPC_OPTIONS_PREFIX.'_forum_lock') * 60);
		$topic_started = $wpdb->get_var($wpdb->prepare("SELECT topic_started FROM ".$wpdb->prefix."cpcommunitie_topics WHERE tid = %d", $tid));
		if ( (get_option(CPC_OPTIONS_PREFIX.'_forum_lock')==0) || ($topic_started > $now) || (current_user_can('level_10')) ) {
			return false;
		} else {
			return true;
		}
	...
	function add_reply($tid, $reply_text, $uid=0, $replybyemail=false) {
		...
		if ($tid != '') {
			...
			$cat_id = $wpdb->get_var($wpdb->prepare("SELECT topic_category from ".$wpdb->prefix."cpcommunitie_topics where tid = %d", $tid));
			...
			$sql = "SELECT tid FROM ".$wpdb->prefix."cpcommunitie_topics WHERE topic_parent = %d AND topic_post = %s";
					...
					if ($topic_approved == "on") {
						...
						if ($query) {						
							foreach ($query as $user) {	
								...
								apply_filters ('__cpc__forum_newreply_filter', $user->ID, $uid, $owner_name, $url);
							...
							foreach ($query as $user) {	
								...
								// Get role of recipient user
									...
									if (strpos(strtolower($cat_roles), 'everyone,') !== FALSE || strpos(strtolower($cat_roles), $user_role.',') !== FALSE) {	 
										...
										__cpc__sendmail($user->user_email, $subject, $body);							
