...
class cpc_mail {
	...
	function get_inbox_count($unread_only=false, $sent_items=false, $term='') {
		...
		$sql = "SELECT COUNT(mail_mid) AS cnt FROM ".$wpdb->base_prefix."cpcommunitie_mail m
	...
	function get_inbox($count=10, $start=0, $avatar_size=40, $term="", $order=true, $message_len=75, $unread_only=false, $sent_items=false) {
		...
		if (!$sent_items) {
		    $sql = "SELECT m.mail_mid, m.mail_from, m.mail_to, m.mail_read, m.mail_sent, m.mail_subject, m.mail_message, u.display_name
	    		FROM ".$wpdb->base_prefix."cpcommunitie_mail m
	    		INNER JOIN ".$wpdb->base_prefix."users u ON m.mail_from = u.ID
	 	...
	 	foreach ($messages AS $item) {
	 	    ...
	 	    if ($continue) {
				...
				if (!$sent_items) {
					$row_array['display_name_link'] = stripslashes(__cpc__profile_link($item->mail_from));
					$row_array['avatar'] = get_avatar($item->mail_from, $avatar_size);
				} else {
					$row_array['display_name_link'] = stripslashes(__cpc__profile_link($item->mail_to));
	...
	function get_message($mail_id) {
		if (is_numeric($mail_id)) {
			global $wpdb;
			$sql= "SELECT m.*, u.display_name FROM ".$wpdb->base_prefix."cpcommunitie_mail m
	    		INNER JOIN ".$wpdb->base_prefix."users u ON m.mail_from = u.ID
	    		WHERE m.mail_mid = %d";
	...
	function set_as_read($mail_mid) {
		if (is_numeric($mail_mid)) {
			global $wpdb;
			$sql = "UPDATE ".$wpdb->base_prefix."cpcommunitie_mail SET mail_read = 'on' WHERE mail_mid = %d";
	...
	function set_as_deleted($mail_mid) {
		if (is_numeric($mail_mid)) {
			...
			if ( is_user_logged_in() ) {
				$sql = "SELECT mail_to, mail_from FROM ".$wpdb->base_prefix."cpcommunitie_mail WHERE mail_mid = %d";
	...
	function sendmail($compose_recipient_id) {
		...
		if (is_user_logged_in()) {
			...
			if (!$recipient) {
				...
				// subject and message from cpc_ui elements
				$subject = $_POST['cpc-mail-subject'];
				$message = $_POST['cpc-mail-message'];
				...
				if ( __cpc__get_meta($recipient->ID, 'notify_new_messages') ) {
					...
					$body = "<h1>".$subject."</h1>";
					$body .= "<p><a href='".$url.__cpc__string_query($url)."mid=".$mail_id."'>".sprintf(__("Gehe zu %s Mail", 'cp-communitie'), __cpc__get_url('mail'))."...</a></p>";
					$body .= "<p>";
					$body .= $message;
					$body .= "</p>";
					$body .= "<p><em>";
