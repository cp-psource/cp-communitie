...
class cpc_ui {
	...
	function mail_subject($subject_text="Mail subject", $box_class="") {
		$html = '<input type="text" name="cpc-mail-subject" class="'.$box_class.'" onblur="this.value=(this.value==\'\') ? \''.$subject_text.'\' : this.value;" onfocus="this.value=(this.value==\''.$subject_text.'\') ? \'\' : this.value;" value="'.$subject_text.'" />';
		return $html;
	}
	
	function mail_message($message_text="Message...", $box_class="") {
		$html = '<textarea name="cpc-mail-message" class="'.$box_class.'" onblur="this.value=(this.value==\'\') ? \''.$message_text.'\' : this.value;" onfocus="this.value=(this.value==\''.$message_text.'\') ? \'\' : this.value;" value="'.$message_text.'">'.$message_text.'</textarea>';
		return $html;
	}
	...
	function whatsup($whatsup_text='', $box_class='input-field') {
		$whatsup = $whatsup_text ? $whatsup_text : get_option(CPC_OPTIONS_PREFIX.'_status_label');		
		return '<input type="text" id="__cpc__comment" name="status" class="'.$box_class.'" onblur ="this.value=(this.value==\'\') ? \''.addslashes($whatsup_text).'\' : this.value;" onfocus="this.value=(this.value==\''.addslashes($whatsup_text).'\') ? \'\' : this.value;" value="'.stripslashes($whatsup_text).'" />';
	}
	...
	function get_breadcrumbs($current_catid=0, $param='catid', $show_home=true, $sep='&rarr;', $include_last_link=false, $return_array='', $url='', $order=100) {
		...
		$url = $url != '' ? $url : get_bloginfo('url').get_option(CPC_OPTIONS_PREFIX.'_forum_url');
		...
		$sql = "select * from ".$wpdb->prefix."cpcommunitie_cats where cid = %d";
		...
		if (!$parent_cat) {
			...
			if ($parent_cat->cat_parent > 0) {
				...
				$trail = __cpc__sub_val_sort($return_array,'order');
				$crumbs = '';
				$count = 0;
				foreach ($trail as $crumb) {
					$count++;
					if ($count < count($trail) || $include_last_link) {
						$crumbs .= "<a href='".$url.__cpc__string_query($url).$param."=".$crumb['this_id']."'>".$crumb['cat_title']."</a> ";
