...
class cpc_user {
	...
	public function __construct($id='') {
		...
		if ($user_info) {
			...
			$this->activity_privacy = __cpc__get_meta($id, 'wall_share');	// Privacy for sharing activity
			$this->dob_day = __cpc__get_meta($id, 'dob_day');				// Date of Birth (day)
			$this->dob_month = __cpc__get_meta($id, 'dob_month');			// Date of Birth (month)
			$this->dob_year = __cpc__get_meta($id, 'dob_year');				// Date of Birth (year)		
			$this->last_activity = __cpc__get_meta($id, 'last_activity');	// When last active		
		}
	...
	function set_city($value) {
    	$this->city = $value;
		__cpc__update_meta($this->id, 'city', $value);
    }
    function get_city() {
		return $this->city;
    }
	...
	function set_country($value) {
    	$this->country = $value;
		__cpc__update_meta($this->id, 'country', $value);
    }
    ...
    function get_latest_activity($as_row=false) {
        global $wpdb;
        if (!$as_row) {
	        $comment = stripslashes($wpdb->get_var( $wpdb->prepare("SELECT comment FROM ".$wpdb->base_prefix."cpcommunitie_comments WHERE subject_uid = %d AND author_uid = %d AND comment_parent = 0 ORDER BY cid DESC LIMIT 0,1", $this->id, $this->id)));
        } else {
            $comment = $wpdb->get_row( $wpdb->prepare("SELECT comment FROM ".$wpdb->base_prefix."cpcommunitie_comments WHERE subject_uid = %d AND author_uid = %d AND comment_parent = 0 ORDER BY cid DESC LIMIT 0,1", $this->id, $this->id));
        }
		return $comment;
    }        
    function get_latest_activity_age() {
        global $wpdb;
		$dateof = $wpdb->get_var( $wpdb->prepare("SELECT comment_timestamp FROM ".$wpdb->base_prefix."cpcommunitie_comments WHERE subject_uid = %d AND author_uid = %d AND comment_parent = 0 ORDER BY cid DESC LIMIT 0,1", $this->id, $this->id));
		return $dateof;
    }        
	function set_activity_privacy($value) {
    	$this->activity_privacy = $value;
		__cpc__update_meta($this->id, 'wall_share', $value);
    }
	...
	function set_dob_day($value) {
    	$this->dob_day = $value;
		__cpc__update_meta($this->id, 'dob_day', $value);
    }
	...
	function set_dob_month($value) {
		...
		__cpc__update_meta($this->id, 'dob_month', $value);
    }
	...
	function set_dob_year($value) {
    	$this->dob_year = $value;
		__cpc__update_meta($this->id, 'dob_year', $value);
    }
    ...
    function get_user_meta($uid, $slug) {
		return __cpc__get_meta($uid, $slug);
    }
    ...
    function get_extended() {
        ...
        $sql = "SELECT * FROM ".$wpdb->base_prefix."usermeta where user_id = %d and meta_key like 'cpcommunitie_extended%%'";
        $values = $wpdb->get_results($wpdb->prepare($sql, $this->id));
		if ($values) {
			$ext_rows = array();
			foreach ($values as $value) {
				$slug = str_replace('cpcommunitie_extended_', '', $value->meta_key);
				$sql = "SELECT * FROM ".$wpdb->base_prefix."cpcommunitie_extended WHERE extended_slug = %s";
    ...
    function get_following($max=10) {
	    ...
	    $sql = "SELECT f.following AS id FROM ".$wpdb->base_prefix."cpcommunitie_following f
    ...
    function get_groups($max=10) {
	    ...
	    $sql = "SELECT f.group_id AS id, g.last_activity FROM ".$wpdb->base_prefix."cpcommunitie_group_members f
	    		LEFT JOIN ".$wpdb->base_prefix."cpcommunitie_groups g ON f.group_id = g.gid
	 	...
	 	if ($results) {
			...
			$list = __cpc__sub_val_sort($list, 'last_activity', false);
		} else {
			$list = false;
		}
	 	return $list;
    ...
    function get_friends($max=10) {
	    ...
	    $sql = "SELECT f.friend_to AS id, u.meta_value as last_activity FROM ".$wpdb->base_prefix."cpcommunitie_friends f
	    		LEFT JOIN ".$wpdb->base_prefix."usermeta u ON f.friend_to = u.user_id
	    		WHERE friend_from = %d AND friend_accepted = 'on' AND u.meta_key = 'cpcommunitie_last_activity'
    ...
    function get_friend_requests($max=10) {
	    ...
	    $sql = "SELECT f.friend_from AS id, friend_timestamp FROM ".$wpdb->base_prefix."cpcommunitie_friends f
	...
	function get_profile_url($just_url=false) {
		return __cpc__profile_link($this->id, $just_url);
	}
	...
	function is_friend() {
	   	global $wpdb,$current_user;
		if ( $wpdb->get_var($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix."cpcommunitie_friends WHERE (friend_accepted = 'on') AND (friend_from = %d AND friend_to = %d)", $this->id, $current_user->ID)) ) {
			return true;
		} else {
			return false;
		}
...
function user_has_permission($id, $type) {
	...
	$privacy = __cpc__get_meta($id, $type);
	if (is_user_logged_in() || $privacy == 'public') {	
		$is_friend = __cpc__friend_of($id, $current_user->ID);
		if ((CPC_CURRENT_USER_PAGE == $current_user->ID) || (is_user_logged_in() && strtolower($privacy) == 'everyone') || (strtolower($privacy) == 'public') || (strtolower($privacy) == 'friends only' && $is_friend) || __cpc__get_current_userlevel() == 5) {
			return true;
		}
	}
	return false;
