...
/*
CP Community Groups
Description: Groups Directory and Page plugin compatible with CP Community. Put [cpcommunitie-groups] and [cpcommunitie-group] on any WordPress page.
*/
		...
		if ($gid != '') {
			...
			$html = "<div class='__cpc__wrapper'>";
				...
				if (is_user_logged_in()) {
					...
					if ($member_of != "yes") {
						...
						if ($member_of == "no") {
							...
							if ($group->max_members == 0 || $member_count < $group->max_members) {
								if ($group->private != "on") {
									$buttons .='<input type="submit" value="'.__("Join Group", CPC_TEXT_DOMAIN).'" id="groups_join_button" class="__cpc__button">';
									$buttons .='<p id="groups_join_button_done" style="padding:6px;display:none">'.__('You are now a member of this group.', CPC_TEXT_DOMAIN).'</p>';
								} else {
									$buttons .='<input type="submit" value="'.__("Request to Join", CPC_TEXT_DOMAIN).'" id="groups_join_button" class="__cpc__button">';
									$buttons .='<p id="groups_join_button_done" style="padding:6px;display:none">'.__('Your membership is awaiting approval.', CPC_TEXT_DOMAIN).'</p>';
								}
							} else {
								$buttons .='<p>'.__('Group membership is full.', CPC_TEXT_DOMAIN).'</p>';
							}

						} else {
							
							// Asked to join, waiting for decision
							$buttons .= "<p>".__("Your request to join is awaiting approval.", CPC_TEXT_DOMAIN)."</p>";
						...
						if (__cpc__group_admin($gid) != "yes") {
							...
							$buttons .='<input type="submit" value="'.__("Leave Group", CPC_TEXT_DOMAIN).'" id="groups_leave_button" class="__cpc__button">';
							$buttons .='<p id="groups_leave_button_done" style="padding:6px;display:none">'.__('You are no longer a member of this group.', CPC_TEXT_DOMAIN).'</p>';
						}
					...
					if (__cpc__group_admin($gid) == "yes" || __cpc__get_current_userlevel() == 5) {
						...
						if (__cpc__get_current_userlevel() == 5) {
							$buttons .= '<input type="submit" value="'.__("Delete Group", CPC_TEXT_DOMAIN).'" id="groups_delete_button" class="__cpc__button">';
							$buttons .='<p id="groups_delete_button_done" style="padding:6px;display:none">'.__('Group deleted.', CPC_TEXT_DOMAIN).'</p>';
						} else {
							$buttons .= '<input type="submit" title="'.$gid.'" value="'.__("Delete Group", CPC_TEXT_DOMAIN).'" id="groups_delete_button_request" class="__cpc__button">';
						}
					} 
				...
				if (strpos($template, '[menu]') !== false) {
					// vertical menu
					$menu = "";
					$menu .= '<div id="group_menu_all" class="__cpc__group_menu">'.__('All Groups', CPC_TEXT_DOMAIN).'</div>';
					$menu .= '<div id="group_menu_about" class="__cpc__group_menu">'.__('Welcome page', CPC_TEXT_DOMAIN).'</div>';
					if ($member_of == "yes" || $group->content_private != "on") {
						$menu .= '<div id="group_menu_wall" class="__cpc__group_menu">'.__('Group Activity', CPC_TEXT_DOMAIN).'</div>';
						if ($group->group_forum == "on") {
							$menu .= '<div id="group_menu_forum" class="__cpc__group_menu">'.__('Group Forum', CPC_TEXT_DOMAIN).'</div>';
						}
						$menu .= '<div id="group_menu_members" class="__cpc__group_menu">'.__('Active Members', CPC_TEXT_DOMAIN).'</div>';
					}
					if (__cpc__group_admin($gid) == "yes" || __cpc__get_current_userlevel() == 5) {
						$menu .= '<div id="group_menu_settings" class="__cpc__group_menu">'.__('Group Settings', CPC_TEXT_DOMAIN).'</div>';
						if (get_option(CPC_OPTIONS_PREFIX.'_group_invites') == 'on') {
							$menu .= '<div id="group_menu_invites" class="__cpc__group_menu">'.__('Group Invites', CPC_TEXT_DOMAIN).'</div>';
						}
					}
				...
				if ($member_of == "yes" || $group->content_private != "on") {
					...
					$private_link = '';
					if (!is_user_logged_in()) {
						$private_link .= " <a href=".wp_login_url( $group_url.$q.'gid='.$gid )." class='simplemodal-login' title='".__("Login", CPC_TEXT_DOMAIN)."'>".__("Login", CPC_TEXT_DOMAIN).".</a>";
					}
			...
			$html = __("Group not found, sorry.", CPC_TEXT_DOMAIN);
		}
		
	} else {
		
		$html = __cpc__show_login_link(__("You need to <a href='%s'>login</a> to access this group.", CPC_TEXT_DOMAIN));
...
function __cpc__groups() {	
	...
	$html = '<div class="__cpc__wrapper">';
		...
		if ( (is_user_logged_in()) && ($group_all_create == "on" || __cpc__get_current_userlevel() == 5) ) {
			...
			$html .= "<input type='submit' id='show_create_group_button' class='__cpc__button' value='".__("Create Group", CPC_TEXT_DOMAIN)."'>";

			$html .= "<div id='create_group_form' style='display:none'>";
				$html .= "<div>";
				$html .= "<strong>".__("Name of Group", CPC_TEXT_DOMAIN)."</strong><br />";
				$html .= "<input type='text' id='name_of_group' class='new-topic-subject-input' style='width: 98% !important;'>";
				$html .= "</div>";

				$html .= "<div>";
				$html .= "<strong>".__("Description", CPC_TEXT_DOMAIN)."</strong><br />";
				$html .= "<input type='text' id='description_of_group' style='width: 98% !important;'>";
				$html .= "</div>";

				$html .= "<div style='margin-top:10px'>";
				$html .= "<input type='submit' id='create_group_button' class='__cpc__button' value='".__("Create", CPC_TEXT_DOMAIN)."'>";
				$html .= "<input type='submit' id='cancel_create_group_button' class='__cpc__button' value='".__("Cancel", CPC_TEXT_DOMAIN)."'>";
				$html .= "</div>";
			$html .= "</div>";
		...
		if ( $term != '' ) {
			...
			$html .= '<input id="groups_go_button" type="submit" class="__cpc__button" value="'.__("Search", CPC_TEXT_DOMAIN).'" />';
			$html .= "</div>";	
			...
			if ($groups) {
				...
				foreach ($groups as $group) {
					...
					if (__cpc__member_of($group->gid) == 'yes') { 
						...
						$name = stripslashes($group->name) != '' ? stripslashes($group->name) : __('[No name]', CPC_TEXT_DOMAIN);
		...
		if (isset($groups) && !$groups) 
				$html .= "<div style='clear:both'>".__("No group found....", CPC_TEXT_DOMAIN)."</div>";
...
function __cpc__add_groups_to_admin_menu()
{
	...
	add_submenu_page('cpcommunitie_debug'.$hidden, __('Groups', CPC_TEXT_DOMAIN), __('Groups', CPC_TEXT_DOMAIN), 'manage_options', CPC_DIR.'/groups_admin.php');
}
