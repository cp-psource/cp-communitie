...
Directory=members';
	...
	<table class="form-table __cpc__admin_table"> 
		...
		<tr valign="top"> 
		<td scope="row"><label for="cpc_use_group_templates"><?php echo __('Custom Group Page templates', CPC_TEXT_DOMAIN); ?></label></td>
		<td>
		...
		<span class="description"><?php echo sprintf(__('Activate <a href="%s">templates</a> for the group page (default layout used if not)', CPC_TEXT_DOMAIN), 'admin.php?page=cpcommunitie_templates#group_options'); ?></span></td> 
		</tr> 

		<tr valign="top"> 
		<td scope="row"><label for="group_all_create"><?php _e('All users can create', CPC_TEXT_DOMAIN); ?></label></td>
		<td>
		<input type="checkbox" name="group_all_create" id="group_all_create" <?php if ($group_all_create == "on") { echo "CHECKED"; } ?>/>
		<span class="description"><?php echo __('All users or restricted to administrators only', CPC_TEXT_DOMAIN); ?></span></td> 
		</tr> 

		<tr valign="top"> 
		<td scope="row"><label for="initial_groups"><?php _e('Default Groups', CPC_TEXT_DOMAIN); ?></label></td> 
		<td><input name="initial_groups" type="text" id="initial_groups"  value="<?php echo $initial_groups; ?>" /> 
		<span class="description"><?php echo __('Comma separated list of group ID\'s that new members are assigned to (leave blank for none)', CPC_TEXT_DOMAIN); ?></td> 
		</tr> 
		
		<tr valign="top"> 
		<td scope="row"><label for="group_invites"><?php _e('Allow group invites', CPC_TEXT_DOMAIN); ?></label></td>
		<td>
		<input type="checkbox" name="group_invites" id="group_invites" <?php if ($group_invites == "on") { echo "CHECKED"; } ?>/>
		<span class="description"><?php echo __("Allow group admin's to invite people to join via email.", CPC_TEXT_DOMAIN); ?></span></td> 
		</tr> 

		<tr valign="top"> 
		<td scope="row"><label for="group_max_members"><?php _e('Default maximum members', CPC_TEXT_DOMAIN); ?></label></td>
		...
		<span class="description">
			<?php echo __('Maximum number of members a new group allows (can be changed in group settings), 0=unlimited.', CPC_TEXT_DOMAIN); ?>
		</span></td> 
		</tr> 

		<tr valign="top"> 
		<td scope="row"><label for="group_invites_max"><?php _e('Maximum invitations', CPC_TEXT_DOMAIN); ?></label></td>
		...
		<span class="description">
			<?php echo __('How many invitations to join the group can be sent out at one time (to avoid spamming from your server).', CPC_TEXT_DOMAIN); 
			__('Note: If people who are invited to join via email are not members they will be able to register first (if the option is set in WordPress).', CPC_TEXT_DOMAIN); ?>
		</span></td> 
		</tr> 
	...
	<tr><td colspan="2"><h2><?php _e('Group Menu Items', CPC_TEXT_DOMAIN) ?></h2></td></tr>

	<tr valign="top"> 
	<td scope="row"><label for="group_invites_max"><?php _e('Menu structure', CPC_TEXT_DOMAIN); ?></label></td>
	<td>
	...
	<span class="description"><?php echo sprintf(__('Only applicable to the horizontal version of the group page menu, set on the Plus options tab.', CPC_TEXT_DOMAIN), CPC_WL); ?></span><br />
	<a id="__cpc__reset_group_menu" href="javascript:void(0)"><?php echo __('Reset the above...', CPC_TEXT_DOMAIN); ?></a>
	</td> 
	</tr> 
	...
	echo '<h2>'.__('Delete group / manage group members', CPC_TEXT_DOMAIN).'</h2>';

	echo '<p style="margin-left:10px">';	
	echo __("Select a group to show current members. Then type part of a member's display name or username to search. Keep blank for all users.", CPC_TEXT_DOMAIN).'<br />';
	echo __("You cannot add or remove the group administrator. Group administrators are not displayed.", CPC_TEXT_DOMAIN).'<br />';
	echo '</p>';
	...
	if ($groups) {
		...
		echo '<option value=0>'.__('-- Select a group --', CPC_TEXT_DOMAIN).'</option>';
		...
		echo '<input type="submit" id="user_list_search_button" name="Submit" class="button-primary" value="'.__('Search', CPC_TEXT_DOMAIN).'" />';
		echo '</div>';
		...
		echo '<a href="javascript:void(0)" id="group_list_delete_link">'.__('Delete this group', CPC_TEXT_DOMAIN).'</a>';
		echo '</div>';
		echo '<div id="group_order_update" style="margin-left:10px; display:none;">';
		echo '<a href="javascript:void(0)" id="group_order_update_link">'.__('Change this group&apos;s order', CPC_TEXT_DOMAIN).'</a>';
		echo '</div>';
		
		echo '<div style="clear:both; margin:10px; float:left;">';
		echo '<strong>'.__('Available users', CPC_TEXT_DOMAIN).'</strong><br />';
		...
		echo '<strong>'.__('Group members', CPC_TEXT_DOMAIN).'</strong><br />';
		...
		echo '<input type="submit" id="users_add_button" name="Submit" class="button-primary" value="'.__('Update', CPC_TEXT_DOMAIN).'" />';
		echo '</div>';
		...
		<table style="margin-left:10px; margin-top:10px;">						
			...
			<tr><td width="165px">[<?php echo CPC_SHORTCODE_PREFIX; ?>-group]</td>
				<td><?php echo __('Used to display a group page, should not be included in user navigation or menu.', CPC_TEXT_DOMAIN); ?></td></tr>
			<tr><td width="165px">[<?php echo CPC_SHORTCODE_PREFIX; ?>-groups]</td>
				<td><?php echo __('Display the groups on the site.', CPC_TEXT_DOMAIN); ?></td></tr>
		</table>
		<?php 
		...
		echo '<p style="margin-left:10px">';
		echo __('No groups created yet.', CPC_TEXT_DOMAIN);
		echo '</p>';
