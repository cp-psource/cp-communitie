...
if (is_user_logged_in() && isset($_GET['u'])) {
	...
	<div class="login" style="width:400px; margin: 80px auto;">
		...
		<form name="loginform" id="loginform" action="<?php echo site_url('wp-login.php', 'login_post') ?>" method="post">
			<p><?php _e('Um fortzufahren, melde Dich bitte an oder registriere Dich.', 'cp-communitie'); ?></p>
			<p> 
				<label><?php _e('Benutzername', 'cp-communitie') ?><br /> 
			...
			<p class="submit"> 
				<input type="submit" name="wp-submit" id="wp-submit" class="button-primary" value="<?php esc_attr_e('Anmeldung', 'cp-communitie'); ?>" tabindex="100" />
		...
		<a href="<?php echo site_url('wp-login.php?action=register', 'login') ?>"><?php _e('Registrieren', 'cp-communitie') ?></a> |
		<a href="<?php echo site_url('wp-login.php?action=lostpassword', 'login') ?>" title="<?php _e('Passwort verloren und gefunden', 'cp-communitie') ?>"><?php _e('Passwort vergessen?', 'cp-communitie') ?></a>
		<?php else : ?>
		<a href="<?php echo site_url('wp-login.php?action=lostpassword', 'login') ?>" title="<?php _e('Passwort verloren und gefunden', 'cp-communitie') ?>"><?php _e('Passwort vergessen?', 'cp-communitie') ?></a>
		<?php endif; ?>
		</p>
