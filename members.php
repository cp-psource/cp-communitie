...
function __cpc__members($attr) {	

	global $wpdb, $current_user;
	wp_get_current_user();

	$plugin = CPC_PLUGIN_URL;
	$dbpage = $plugin.'/cpcommunitie_members_db.php';
	...
	$html = '<div class="__cpc__wrapper">';
		...
		if (!is_user_logged_in() && get_option(CPC_OPTIONS_PREFIX.'dir_hide_public') ) {
			...
			echo __cpc__show_login_link(__("Du musst <a href='%s'>eingeloggt</a> sein, um das Verzeichnis anzuzeigen.", 'cp-communitie'));
			...
			if ($roles) {
				...
				$dir_levels = strtolower(get_option(CPC_OPTIONS_PREFIX.'_dir_level'));
			...
			$html .= '<div id="__cpc__directory_roles" style="display:none">'.$dir_levels.'</div>';
			
			// Stores start value for more
			$start = get_option(CPC_OPTIONS_PREFIX.'_dir_page_length')+1;
			$html .= '<div id="cpcommunitie_directory_start" style="display:none">'.$start.'</div>';
			$html .= '<div id="cpcommunitie_directory_page_length" style="display:none">'.get_option(CPC_OPTIONS_PREFIX.'_dir_page_length').'</div>';
			...
			$html .= "<div class='members_row' style='padding:0px'>";
				...
				$html .= '<input id="members_go_button" type="submit" class="__cpc__button" value="'.__("Suche", 'cp-communitie').'" />';
				if (is_user_logged_in()) {
					$html .= '<div style="clear:both;"><input type="checkbox" id="cpcommunitie_member_friends" /> '.__('Nur Freunde', 'cp-communitie').'</div>';
				}
				$html .= '</div>';	
				...
				if (!get_option(CPC_OPTIONS_PREFIX.'_cpc_lite') && function_exists('__cpc__profile_plus')) {
					...
					$html .= '<a href="javascript:void(0);" id="cpcommunitie_show_advanced" /> '.__('Erweiterte Suche', 'cp-communitie').'</a>';
					$html .= '</div>';
				}
			$html .= "</div>";
			...
			if (!get_option(CPC_OPTIONS_PREFIX.'_cpc_lite') && function_exists('__cpc__profile_plus')) {
				...
				$extensions = $wpdb->get_results("SELECT * FROM ".$wpdb->base_prefix."cpcommunitie_extended WHERE search = 'on' ORDER BY extended_order, extended_name");

				if ($extensions) {

					$html .= "<div id='cpcommunitie_advanced_search' style='width:90%;padding:0px;display:none;'>";
					...
					foreach ($extensions as $extension) {
						...
						$html .= '<tr>';
							...
							if ($extension->extended_type == 'Checkbox') {
								$html .= '<td id="__cpc__ext_label_'.$extension->eid.'" style="border:0">';
								$html .= stripslashes($extension->extended_name);
								$html .= '</td><td id="__cpc__ext_value_'.$extension->eid.'" style="border:0">';
								$html .= '<input rel="checkbox" id="'.$extension->eid.'" class="cpcommunitie_extended_search" type="checkbox" name="extended_value[]" />';
								$html .= '</td>';
							}
							if ($extension->extended_type == 'List') {
								...
								$html .= '<option value="'.__('Beliebig', 'cp-communitie').'">'.__('Beliebig', 'cp-communitie').'</option>';
			...
			// Sort by option
			$order = get_option(CPC_OPTIONS_PREFIX.'_dir_atoz_order');
				...
				$html .= '<select id="cpcommunitie_members_orderby">';
					...
					if (get_option(CPC_OPTIONS_PREFIX.'_use_distance') && function_exists('__cpc__profile_plus') && !get_option(CPC_OPTIONS_PREFIX.'_hide_location')) {
						$html .= '<option value="distance"';
							...
							$html .= '>'.__('Distanz', 'cp-communitie').'</option>';
					}
				$html .= '</select>';
			$html .= '</div>';
			
			// A to Z
			$html .= '<div id="cpcommunitie_members_atoz">';
				for ($i = 65; $i <= 90; $i++) { 
					if (chr($i) != strtoupper($term)) {
						// Get directory URL worked out
						$member_url = __cpc__get_url('members');
						$q = __cpc__string_query($member_url);
			...
			$html .= '<div id="__cpc__members">';

				$search_limit = 1000;
				$sql_ext = strlen($term) != 1 ? "OR (lower(u.display_name) LIKE '% %".$term."%')" : "";
				
				$lat = __cpc__get_meta($current_user->ID, 'plus_lat');
				if (get_option(CPC_OPTIONS_PREFIX.'_use_distance') && $lat != 0 && is_user_logged_in() && function_exists('__cpc__profile_plus')) {
				...
				if ($members) {
					...
					if (CPC_DEBUG) $html .= '<p>Processing $members.</p>';
								...
								$html .= "<div class='members_row";
									...
									$html .= "<div class='members_info'>";
										...
										$html .= "<div style='padding-left: 75px;'>";						
											...
											if (function_exists('__cpc__mail') && !get_option(CPC_OPTIONS_PREFIX.'_cpc_lite')) {
												// Show Send Mail button
												if (get_option(CPC_OPTIONS_PREFIX.'_show_dir_buttons') && $member->uid != $current_user->ID) {
													if ($is_friend) {
														// A friend
											...
											$html .= __('letzte Aktivität', 'cp-communitie').' '.__cpc__time_ago($member->last_activity).". ";
											...
											if (function_exists('__cpc__profile_plus') && is_user_logged_in() && $member->distance < 99999 && $member->uid != $current_user->ID) {
												...
												if ( (strtolower($share) == 'everyone') 
													...
													if ($measure != 'on') { 
														...
														$miles = __('Meilen', 'cp-communitie');
													}	
													$html .= '<br />'.__('Distanz', 'cp-communitie').': '.$distance.' '.$miles;
													if ($show_alt == 'on') {
														if ($measure != 'on') { 
															$html .= ' ('.intval(($distance/8)*5).' '.__('Meilen', 'cp-communitie').')';
														} else {
											...
											if (!get_option(CPC_OPTIONS_PREFIX.'_cpc_lite')) {
												...
												if ( (strtolower($wall_share) == 'everyone') 
													...
													if ($comment) {
														$html .= '<div style="max-height:250px">'.__cpc__buffer(__cpc__make_url(stripslashes($comment->comment))).'</div>';
													}
											...
											if (is_user_logged_in() && get_option(CPC_OPTIONS_PREFIX.'_show_dir_buttons') && $member->uid != $current_user->ID) {
												if (__cpc__pending_friendship($member->uid)) {
													// Pending
													$html .= sprintf(__('%s Anfrage gesendet.', 'cp-communitie'), get_option(CPC_OPTIONS_PREFIX.'_alt_friend'));
												} else {
													if (!$is_friend) {
														// Not a friend
														$html .= '<div id="addasfriend_done1_'.$member->uid.'">';
														$html .= '<input class="add_as_friend_message addfriend_text" title="'.$member->uid.'" id="addtext_'.$member->uid.'" type="text" onclick="this.value=\'\'" value="'.sprintf(__('Als %s hinzufügen...', 'cp-communitie'), get_option(CPC_OPTIONS_PREFIX.'_alt_friend')).'">';
														$html .= '<input type="submit" title="'.$member->uid.'" class="addasfriend __cpc__button" value="'.__('Hinzufügen', 'cp-communitie').'" /> ';						
														$html .= '</div>';
														$html .= '<div id="addasfriend_done2_'.$member->uid.'" class="hidden">'.sprintf(__('%s Anfrage gesendet', 'cp-communitie'), get_option(CPC_OPTIONS_PREFIX.'_alt_friend')).'</div>';	
													}
												}
											}
					...
					$html .= "<div id='showmore_directory_div' style='text-align:center; width:100%'><a href='javascript:void(0)' id='showmore_directory'>".__("mehr...", 'cp-communitie')."</a></div>";

				} else {
					$html .= '<br />'.__('Keine Mitglieder gefunden', 'cp-communitie')."....";
				} // if ($members)

			}
			
		$html .= '</div>'; // __cpc__members
		
	$html .= '</div>'; // __cpc__wrapper

	// Filter for header
	$html = apply_filters ( 'cpcommunitie_member_header_filter', $html );
...
/* ====================================================== SET SHORTCODE ====================================================== */
if (!is_admin()) {
	add_shortcode(CPC_SHORTCODE_PREFIX.'-members', '__cpc__members');  
}
