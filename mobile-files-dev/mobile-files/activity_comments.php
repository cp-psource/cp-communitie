...
if (isset($_POST['post_comment']) && $_POST['post_comment'] != '' && $_POST['post_comment'] != __('Write a comment...', 'cp-communitie') && $current_user->ID > 0) {
	...
	$wpdb->query( $wpdb->prepare( "
		...
		$subject_name = $wpdb->get_var($wpdb->prepare("SELECT display_name FROM ".$wpdb->base_prefix."users WHERE ID = %d", $cpc_user->id));
		...
		if ($parent_post_recipient) {
			if (__cpc__get_meta($parent_post_recipient->ID, 'notify_new_wall') == 'on') {
				...
				$url = $profile_url."uid=".$cpc_user->id."&post=".$parent_post->cid;
...
if ( !is_user_logged_in() ) {
	...
	if ($tid) {
		...
		// Show first post		
		$post = $cpc_user->get_activity_post($tid);
		echo "<div class='__cpc__profile_activity_div'>";
		echo $post->display_name.' '.__('posted', 'cp-communitie').' ';
		echo __cpc__time_ago($post->comment_timestamp).".<br />";
		echo '<br />'.$post->comment;
		
		// Check for any associated uploaded images for activity
		$directory = WP_CONTENT_DIR."/cpc-content/members/".$post->author_uid.'/activity/';
		if (file_exists($directory)) {
			$handler = opendir($directory);
			while ($image = readdir($handler)) {
				$path_parts = pathinfo($image);
				if ($path_parts['filename'] == $post->cid) {
					$directoryURL = WP_CONTENT_URL."/cpc-content/members/".$post->author_uid.'/activity/'.$image;
		...
		// Get comments		
		$comments = $cpc_user->get_comments($tid);
