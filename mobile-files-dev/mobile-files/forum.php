...
if ( !is_user_logged_in() ) {
	...
	if (isset($_POST['__cpc__reply_text']) && $_POST['__cpc__reply_text']) {
		$cpc_forum->add_reply($tid, '<p>'.$_POST['__cpc__reply_text'].'</p>');
	}
	...
	if ($tid) {
		...
		if ($topic) {
			...
			echo '<div id="my-forum-topic">';
				echo '<div class="__cpc__threads_title">'.stripslashes($topic->topic_subject).'</div>';
				echo 'Started by '.$topic->display_name.' '.__cpc__time_ago($topic->topic_started).'<br /><br />';
				echo '<div class="__cpc__threads_div">';
			...
			if (get_option(CPC_OPTIONS_PREFIX.'_permalink_structure')) {
				...
				echo "<br />(<a href='".$forum_url.'/'.$perma_cat.$topic->stub."'>".__('view on full site', 'cp-communitie')."</a>)<br /><br />";
			} else {
				echo "<br />(<a href='".$forum_url.$forum_q."cid=".$topic->topic_category."&show=".$topic->tid."'>".__('view on full site', 'cp-communitie')."</a>)<br /><br />";
			}			
			...
			if ($replies) {				
				...
				// Show avatar
					echo '<div class="__cpc__threads_div">';
						...
						// Show comments
						$comments = $cpc_forum->get_replies($reply->tid, 0, 9999, 'ASC');
						if ($comments) {				
							foreach ($comments as $comment) {
							// Show avatar
								echo '<div class="__cpc__comments_div">';
									echo '<strong>';
										echo 'Comment by '.$comment->display_name.' '.__cpc__time_ago($comment->topic_started);
									echo '</strong>';
									$comment_text = __cpc__make_url(stripslashes($comment->topic_post));
									$comment_text = __cpc__bbcode_replace($comment_text);
			...
			echo $cpc_ui->forum_reply('Reply', 'submit small blue', 'textarea', true);
				...
				foreach ($categories as $category) {
					echo '<div class="my-forum-row">';
						echo '<div class="my-forum-row-title">';
							...
							$title .= ' ('.$cpc_forum->get_topics_count($category->cid).')';
						...
						echo '<div style="margin-bottom:5px">';
							$last_topic = $cpc_forum->get_topics($category->cid, 0, 10);
							if ($last_topic) {
								foreach ($last_topic as $topic) {
									...
									echo __cpc__time_ago($topic->topic_started);
									$replies = $cpc_forum->get_replies_count($topic->tid);
			...
			if ($topics) {
				...
				foreach ($topics as $topic) {
					echo '<div class="__cpc__threads_div">';
						...
						echo "<br />".__("Started by", 'cp-communitie').' '.$topic->display_name;
						echo ", ".__cpc__time_ago($topic->topic_date);
						$text = __cpc__make_url(stripslashes($topic->topic_post));
						$text = __cpc__bbcode_replace($text);
						...
						if ($last_reply) {
							...
							$text = __cpc__make_url(stripslashes($last_reply->topic_post));
							$text = __cpc__bbcode_replace($text);
							$text = str_replace('<p>', "<br />", $text);	
							$text = str_replace('</p>', "", $text);	
							$text = str_replace('<br /><br />', "<br />", $text);	
							$text = str_replace(chr(13), "<br />", $text);	
							echo '<hr /><strong>'.__('Last reply by', 'cp-communitie').' '.$last_reply->display_name.' '.__cpc__time_ago($last_reply->topic_date).':</strong><br />';
							echo $text;
						} else {
							echo '<br /><br />'.$text;
							echo '<hr />'.__('No replies', 'cp-communitie');
						}
					echo '</div>';
				}
			} else {
				if ($catid)
					echo __('<br />No topics here.', 'cp-communitie');
			}
