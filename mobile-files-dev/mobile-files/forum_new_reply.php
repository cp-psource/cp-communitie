			...
			if ($topic_approved == "on") {
				...
				if ($query) {						
					foreach ($query as $user) {		
						__cpc__sendmail($user->user_email, __('New Forum Reply', 'cp-communitie'), $body);							
					}
				}						
			} else {
...
<?php
echo '<div id="header">'.get_bloginfo('name').'<div class="home_link"><a href="index.php?'.$a.'">'.__('Home', 'cp-communitie').'</a></div></div>';
echo '<div class="subheading">'.__('Post a reply', 'cp-communitie').'</div>';
...
echo "<ul><li><a href='forum.php?cat_id=".$cat_id."&".$a."'>".__('Back to', 'cp-communitie')." ".$cat_name."...</a></li></ul>";
...
if ( is_user_logged_in() ) {
	...
	echo '<div class="form">';
		...
		echo __('Reply text', 'cp-communitie').'<br />';
		echo '<textarea name="new_reply_post"></textarea><br />';
		echo '<input type="submit" class="submit" value="'.__('Post reply', 'cp-communitie').'" />';
		echo '</form>';
	echo '</div>';
