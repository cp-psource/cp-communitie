...
if (isset($_POST['new_topic_subject']) && $_POST['new_topic_subject'] != '' && $_POST['new_topic_post'] != '') {
	...
	if (strpos(strtolower($cat_roles), 'everyone,') !== FALSE || strpos(strtolower($cat_roles), $user_role.',') !== FALSE) {					
		...
		if (get_option(CPC_OPTIONS_PREFIX.'_moderation') == "on") {
			$topic_approved = "";
		} else {
			$topic_approved = "on";
		}
		...
		if ($topic_approved == "on") {
			...
			if ($query) {					
				foreach ($query as $user) {
					__cpc__sendmail($user->user_email, __('New Forum Topic', 'cp-communitie'), $body);						
				}						
			}
		} else {
		...
		$post = __('Started a new forum topic:', 'cp-communitie').' <a href="'.$url.'">'.$new_topic_subject.'</a>';
...
if ( !is_user_logged_in() ) {
		...
		if ($capabilities) {
			...
			foreach ( $capabilities as $role => $name ) {
				if ($role) {
					...
					if (CPC_DEBUG) $html .= 'Checking global forum (symposium_functions) role '.$role.' against '.$viewer.'<br />';
	...
	if ($can_edit) {
		...
		$new_topic_form .= '<form action="#" method="POST">';
			...
			$new_topic_form .= '<div>'.__("Topic Subject", 'cp-communitie').'</div>';
			$new_topic_form .= '<input class="input_text" type="text" name="new_topic_subject" value="">';
			$new_topic_form .= '<div style="clear:both"></div><br />';
			$new_topic_form .= '<div>'.__("First Post in Topic", 'cp-communitie').'</div>';
			...
			if ($forum_all != 'on') {
				$new_topic_form .= '<input type="checkbox" class="checkbox" name="new_topic_subscribe"> '.__("Tell me when I get any replies", 'cp-communitie').'<br />';
			}
			$new_topic_form .= '</div>';

			$new_topic_form .= '<div style="clear:both"></div><br />';
			$new_topic_form .= '<input id="symposium_new_post" type="submit" class="submit small green" value="'.__("Post new topic", 'cp-communitie').'" />';
		...
		if ($allow_new == 'on') {
			...
			if (__cpc__get_current_userlevel() == 5) $new_topic_form .= sprintf(__('<br />Permissions are set via the WordPress admin dashboard->%s->Options->Forum.', 'cp-communitie'), CPC_WL);	
			$new_topic_forum .= "</p>";
		}
