	...
	if ($posts) {
		...
		foreach ($posts as $post)
		{
			...
			$html .= '<div class="__cpc__threads_div">';
				...
				$html .= '<div><a  class="__cpc__threads_title" href="forum.php?tid='.$post->tid.'&'.$a.'">'.stripslashes($post->topic_subject).'</a></div>';
	
				$html .= '<div>';
				$html .= __('Started by', 'cp-communitie').' '.$post->display_name.', '.__cpc__time_ago($post->topic_date).' '.__('in', 'cp-communitie').' ';
				...
				if ($replies > 0) {
					$last_reply = $cpc_forum->get_replies($post->tid, $start=0, $limit=1, 'DESC');
					...
					$text = str_replace('<br /><br />', "<br />", $text);	
					$text = str_replace(chr(13), "<br />", $text);	
					$html .= '<hr /><strong>'.__('Last reply by', 'cp-communitie').' '.$last_reply->display_name.' '.__cpc__time_ago($last_reply->topic_date).':</strong><br />';
					$html .= $text;
				} else {
					$html .= '<hr />'.__('No replies', 'cp-communitie');
				}
