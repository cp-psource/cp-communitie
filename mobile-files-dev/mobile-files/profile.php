...
if (isset($_POST['post_comment']) && $_POST['post_comment'] != '' && $current_user->ID > 0) {
	...
	?>
	<script>
	window.location.href = "index.php?a=1&uid=<?php echo $cpc->get_current_user_page(); ?>";
	</script>
	<?php
	exit;
...
if ( !is_user_logged_in() ) {
	...
	if ( ($cpc_user->id == $current_user->ID) || (is_user_logged_in() && strtolower($privacy) == 'everyone') || (strtolower($privacy) == 'public') || (strtolower($privacy) == 'friends only' && __cpc__friend_of($cpc_user->id, $current_user->ID)) ) {
		...
		if ($extended) {
			foreach ($extended as $row) {
				if ($row['type'] != 'Checkbox') {
					if ($row['value']) {
						echo '<div class="__cpc__profile_div">';
							echo '<div class="__cpc__profile_label">'.stripslashes($row['name']).'</div>';
							echo stripslashes($row['value']).'<br />';					
						echo '</div>';
					}
				} else {
					if (!$row['value'] && get_option(CPC_OPTIONS_PREFIX.'_profile_show_unchecked') != 'on') {
						// Don't show unchecked checkboxes
					} else {
						echo '<div class="__cpc__profile_div">';
							echo '<span class="__cpc__profile_label">'.$row['name'].'</span> ';
							if ($row['value']) {
								echo '&#10004;';
							} else {
								echo '&#10008;';
		...
		if ($friends) {
			echo '<div id="__cpc__friends_list">';
