...
function __cpc__add_news_to_admin_menu()
{
	...
	add_submenu_page('cpcommunitie_debug'.$hidden, __('Alerts', CPC_TEXT_DOMAIN), __('Alerts', CPC_TEXT_DOMAIN), 'manage_options', CPC_DIR.'/news_admin.php');
}
...
function __cpc__send_poke($message_to, $message_from, $from_name, $poke, $cid) {
	...
	$message = $from_name.__(' has sent you a ', CPC_TEXT_DOMAIN).$poke;
...
function __cpc__news_add_message($message_to, $message_from, $from_name, $mail_id) {
	...
	__cpc__news_add($message_from, $message_to, "<a href='".$url.__cpc__string_query($url)."mid=".$mail_id."'>".__("You have a new message from", CPC_TEXT_DOMAIN)." ".$from_name."</a>");
}
...
function __cpc__news_add_friendrequest($message_to, $message_from, $from_name) {
	$url = __cpc__get_url('profile');
	__cpc__news_add($message_from, $message_to, "<a href='".$url.__cpc__string_query($url)."view=friends'>".sprintf(__("New %s request from", CPC_TEXT_DOMAIN), get_option(CPC_OPTIONS_PREFIX.'_alt_friend'))." ".$from_name."</a>");
}
...
function __cpc__news_add_friendaccepted($message_to, $message_from, $from_name) {
	$url = __cpc__get_url('profile');
	__cpc__news_add($message_from, $message_to, "<a href='".$url.__cpc__string_query($url)."view=friends'>".sprintf(__("%s request accepted by", CPC_TEXT_DOMAIN), get_option(CPC_OPTIONS_PREFIX.'_alt_friend'))." ".$from_name."</a>");
}
...
function __cpc__news_add_newtopic($message_to, $from_id, $from_name, $url) {
	__cpc__news_add($from_id, $message_to, "<a href='".$url."'>".__("Subscribed forum topic by", CPC_TEXT_DOMAIN)." ".$from_name."</a>");
}
...
function __cpc__news_add_newreply($message_to, $message_from, $from_name, $url) {
	if ($message_to != $message_from) {
		__cpc__news_add($message_from, $message_to, "<a href='".$url."'>".__("Subscribed forum reply by", CPC_TEXT_DOMAIN)." ".$from_name."</a>");
	}
}
...
function __cpc__news_add_newreplycomment($message_to, $message_from, $from_name, $url) {
	if ($message_to != $message_from) {
		__cpc__news_add($message_from, $message_to, "<a href='".$url."'>".__("Subscribed forum comment by", CPC_TEXT_DOMAIN)." ".$from_name."</a>");
	}
}
...
function __cpc__news_add_wall_newpost($post_to, $post_from, $from_name) {
	if ($post_to != $post_from) {
		__cpc__news_add($post_from, $post_to, "<a href='".__cpc__get_url('profile')."'>".$from_name." ".__("has posted on your profile.", CPC_TEXT_DOMAIN)."</a>");
	}
}
...
function __cpc__news_add_wall_reply($first_post_subject, $first_post_author, $from_id, $from_name, $url) {
	...
	if ($first_post_subject != $current_user->ID) {
		__cpc__news_add($from_id, $first_post_subject, "<a href='".$url."'>".$from_name." ".__("has replied to a post on your profile", CPC_TEXT_DOMAIN)."</a>");
	} else {
		if ($first_post_author != $current_user->ID) {
			__cpc__news_add($from_id, $first_post_author, "<a href='".$url."'>".$from_name." ".__("has replied to a post you started", CPC_TEXT_DOMAIN)."</a>");
		}
	}
...
function __cpc__news_add_wall_reply_involved_in($post_to, $post_from, $from_name, $url) {
	if ($post_to != $post_from) {
		__cpc__news_add($post_from, $post_to, "<a href='".$url."'>".$from_name." ".__("has replied to a post you are involved in", CPC_TEXT_DOMAIN)."</a>");
	}
}
