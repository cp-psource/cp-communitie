...
if (is_user_logged_in()) {
	...
	// Include CSS from CP Community->Styles->CSS
	echo '<style>';
	...
	if (isset($_POST['uploader_uid'])) {
		...
		if ($_FILES["file"]["name"] != '') {
			...
			if ($uploader_ver == 'forum') {
				if ($blog_id > 1) {
					$tmp_path = get_option(CPC_OPTIONS_PREFIX.'_img_path').'/'.$blog_id.'/forum/'.$uploader_tid.'_'.$current_user->ID.'_tmp/';
				} else {
					$tmp_path = get_option(CPC_OPTIONS_PREFIX.'_img_path').'/forum/'.$uploader_tid.'_'.$current_user->ID.'_tmp/';
				}
				$targetFile =  str_replace('//','/',$tmp_path).$filename;
			}
			if ($uploader_ver == 'activity') {
				$source_path = get_option(CPC_OPTIONS_PREFIX.'_img_path')."/members/".$uploader_uid."/activity_upload/"; // Image has been uploaded to here
				$tmp_path = get_option(CPC_OPTIONS_PREFIX.'_img_path')."/members/".$subject_uid."/activity/pending/";	// Where image is copied until activity page refresh
				$targetFile =  str_replace('//','/',$tmp_path).$filename;
			}
			if ($uploader_ver == 'avatar') {
				$tmp_path = get_option(CPC_OPTIONS_PREFIX.'_img_path')."/members/".$current_user->ID."/profile/";
				$targetFile =  str_replace('//','/',$tmp_path).$filename;
			}
			if ($uploader_ver == 'group_avatar') {
				if ($blog_id > 1) {
					$tmp_path = get_option(CPC_OPTIONS_PREFIX.'_img_path')."/".$blog_id."/groups/".$uploader_gid."/profile/";
				} else {
					$tmp_path = get_option(CPC_OPTIONS_PREFIX.'_img_path')."/groups/".$uploader_gid."/profile/";
				}
				$targetFile =  str_replace('//','/',$tmp_path).$filename;
			}
			if ($uploader_ver == 'gallery') {
				$tmp_path = get_option(CPC_OPTIONS_PREFIX.'_img_path')."/members/".$current_user->ID."/media/".$uploader_aid."/";
				$uniqid = uniqid();
			...
			if ($tmp_path != '') {
				...
				if ($_FILES["file"]["error"] > 0) {
					...
					$allowedExts = ','.get_option(CPC_OPTIONS_PREFIX.'_image_ext').','.get_option(CPC_OPTIONS_PREFIX.'_doc_ext').','.get_option(CPC_OPTIONS_PREFIX.'_video_ext');
					...
					if (!$extAllowed) {
						...
						if (move_uploaded_file($_FILES["file"]["tmp_name"], $targetFile)) {
							...
							if ($uploader_ver == 'gallery') {
								...
								include_once('../../'.CPC_DIR.'/SimpleImage.php');
							   	$image = new __cpc__SimpleImage();
								...
								// NB. show and thumbnail are prefixes to filename
	     						...
	     						INSERT INTO ".$wpdb->base_prefix."cpcommunitie_gallery_items
			     				( 	gid,
	     							name,
	     							owner,
			     					created,
			     		        ...
			     		        // Updated gallery table
	                      		$wpdb->query( $wpdb->prepare( "UPDATE ".$wpdb->prefix."cpcommunitie_gallery SET updated = %s WHERE gid = %d", date("Y-m-d H:i:s"), $uploader_aid  ) );

								// Set album cover if not yet set
								$cover = $wpdb->get_var($wpdb->prepare("SELECT cover FROM ".$wpdb->prefix."cpcommunitie_gallery_items WHERE gid = %d", $uploader_aid));
								if (!$cover) {
									$first_item = $wpdb->get_var($wpdb->prepare("SELECT iid FROM ".$wpdb->prefix."cpcommunitie_gallery_items WHERE gid = %d ORDER BY iid LIMIT 0,1", $uploader_aid));
					      			$wpdb->query( $wpdb->prepare( "UPDATE ".$wpdb->prefix."cpcommunitie_gallery_items SET cover = 'on' WHERE iid = %d", $first_item  ) );			
								}
