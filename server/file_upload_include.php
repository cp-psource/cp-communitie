...
function show_upload_form($uploader_dir, $uploader_url, $ver, $button_text='?', $tid=0, $gid=0, $aid=0, $subject_uid=0, $button_style='') {
	...
	if (get_option(CPC_OPTIONS_PREFIX.'_basic_upload') == "on") {

		$html .= '<iframe id="__cpc__file_upload_iframe" scrolling="no" height="100" width="100%" src="'.plugins_url().'/cp-communitie/server/file_upload_form.php?subject_uid='.$subject_uid.'&uploader_uid='.$current_user->ID.'&uploader_tid='.$tid.'&uploader_gid='.$gid.'&uploader_aid='.$aid.'&uploader_dir='.$uploader_dir.'&uploader_url='.$uploader_url.'&uploader_ver='.$ver.'"></iframe>';

	} else {


	   // The file upload form used as target for the file upload widget
	    $html .= '<form id="__cpc__fileupload" action="#" method="POST" enctype="multipart/form-data">';
	        // The fileupload-buttonbar contains buttons to add/delete files and start/cancel the upload
//	        $html .= '<div class="row fileupload-buttonbar">';
//	            $html .= '<div class="span7">';
	                // The fileinput-button span is used to style the file input field as button
	                $html .= '<div class="__cpc__btn success fileinput-button" style="'.$button_style.'">';
