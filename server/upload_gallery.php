...
if (isset($_POST['action']) && $_POST['action'] == 'after_upload_complete') {
	...
	if (is_user_logged_in()) {
		...
		if (__cpc__upload_gallery_is_logged_in($uid, $user_login, $user_email)) {
			$logged_in = true;
		}
	}
		    ...
		    if ($aid != '') {
				...
				if (get_option(CPC_OPTIONS_PREFIX.'_img_db') == "on") {
					...
					if (copy($upload_file,WP_CONTENT_DIR."/uploads/".$filename)) {
				        ...
				        $show_image = __cpc__scaleImageFileToBlob(WP_CONTENT_DIR."/uploads/".$filename, 'show');
				        ...
				        if ($show_image == '') {
					        ...
					        $thumb_image = __cpc__scaleImageFileToBlob(WP_CONTENT_DIR."/uploads/".$filename, 'thumb');
							...
							// Add uploaded image into database
	 						...
	 						INSERT INTO ".$wpdb->prefix."cpcommunitie_gallery_items
		     				( 	gid,
	 							name,
	 							owner,
		     					created,
						...
						$cover = $wpdb->get_var($wpdb->prepare("SELECT cover FROM ".$wpdb->prefix."cpcommunitie_gallery_items WHERE gid = %d", $aid));
						if (!$cover) {
							$first_item = $wpdb->get_var($wpdb->prepare("SELECT iid FROM ".$wpdb->prefix."cpcommunitie_gallery_items WHERE gid = %d ORDER BY iid LIMIT 0,1", $aid));
					...
					// Do this as may not be process in the same order as sent from jQuery fileupload
					$name = '';
					if (file_exists(dirname($upload_file))) {
						...
						while ($file = readdir($handler)) {
							if ($name == '' && $file != "." && $file != ".." && $file != ".DS_Store" && $file != 'thumbnail') {
								...
								$targetPath = get_option(CPC_OPTIONS_PREFIX.'_img_path')."/members/".$uid."/media/".$aid."/";
								...
								// Check that extension is allowed (set via Manage->Settings->Image extensions)
								...
								if (strpos(get_option(CPC_OPTIONS_PREFIX.'_image_ext'), $ext[sizeof($ext)-1]) > 0) {
									...
									if ($html == '') {			
										if (!copy($upload_file,$targetFile)) {
											...
											// NB. show and thumbnail are prefixes to filename
						     		        ...
						     		        // Updated gallery table
				                      		$wpdb->query( $wpdb->prepare( "UPDATE ".$wpdb->prefix."cpcommunitie_gallery SET updated = %s WHERE gid = %d", date("Y-m-d H:i:s"), $aid  ) );
						     		...
						     		// Not a valid image extension (set via Manage->Settings->Image extensions)
						     	}
...
function __cpc__upload_gallery_is_logged_in($uid, $user_login, $user_email) {
...
function __cpc__scaleImageFileToBlob($file, $size) {
