...
<div id="primary">
	...
	if ( !isset($_GET['mail_id']) ) {
		...
		echo '<div id="my-inbox">';
			...
			$inbox = $cpc_mail->get_inbox(5); // defaults to 10 messages from first available
			...
			if ($inbox) {
				...
				foreach($inbox as $mail) {
					...
					echo '<div class="my-inbox-row '.$mail_unread.'">';
						...
						echo '<div class="my-inbox-sent">'.$d[2].' '.substr(__cpc__get_monthname($d[1]),0,3).' '.$d[0].', '.$t[0].':'.$t[1].'</div>';
		...
		$message = $cpc_mail->get_message($_GET['mail_id']); // Ge message details
