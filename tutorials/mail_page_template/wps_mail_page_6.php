...
<div id="primary">
	...
	if ( ($action == 'reply' && $mail_id) || ($action == 'inbox' || $action == 'sent') ) { 
		...
		echo '<div '.$override_show_state.' id="my-inbox">';
			...
			$inbox = $cpc_mail->get_inbox($page_length, $start, 40, "", true, 75, false, $show_sent_mail);
		...
		if ($pages > 1) {
			...
			if ($current_page > 1) echo '<a href="'.$cpc->get_mail_url().'?action='.$paging_action.'&start=0">First</a> ';

			if ($current_page > 1) echo '<a href="'.$cpc->get_mail_url().'?action='.$paging_action.'&start='.((($current_page-1)*$page_length)-$page_length).'">Previous</a> ';
			...
			for ($p=1; $p<=$pages; $p++) {
				...
				if ($p==$current_page) {
					...
					echo ' <a href="'.$cpc->get_mail_url().'?action='.$paging_action.'&start='.(($p*$page_length)-$page_length).'">'.$p.'</a>';

				}

			}

			if ($current_page < $pages) echo ' <a href="'.$cpc->get_mail_url().'?action='.$paging_action.'&start='.((($current_page+1)*$page_length)-$page_length).'">Next</a>';

			if ($current_page < $pages) echo ' <a href="'.$cpc->get_mail_url().'?action='.$paging_action.'&start='.(($pages*$page_length)-$page_length).'">Last</a>';
