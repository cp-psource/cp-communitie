<?php
/**
 * Template Name: Demo profile page 7
...
<div id="primary">
	...
	echo '<div id="my-sidebar">';
		echo '<div id="my-profile-box">';
			...
			if ($cpc_user->is_permitted('personal')) { // defaults to activity permission, set to "personal" for personal info
				...
				if ($cpc->get_current_user_page() == $current_user->ID) {
					$cpc_mail = new cpc_mail();
					echo '<div id="my-mail-info">';
						echo '<div style="font-weight:bold; margin-bottom:6px;">Mail</div>';
						$unread = $cpc_mail->get_inbox_count(true);
						echo 'You have '.($cnt = $cpc_mail->get_inbox_count()).' message'.($cnt != 1 ? 's' : '').' of which '.$unread.' '.($unread != 1 ? 'are' : 'is').' unread.';
						...
						if ($unread) {
							...
							$recent_mail = $cpc_mail->get_inbox($recent, 0, 18, "", true, 0, true);
	...
	echo '<div id="my-content" class="__cpc__wrapper">';	
		if ($cpc_user->is_permitted()) { 
			...
			echo '<div id="my-menu">';
				if ($cpc_user->get_id() == $current_user->ID) {
					echo '<div id="menu_wall" class="__cpc__profile_menu">Meine Aktivität</div>';
				} else {
					echo '<div id="menu_wall" class="__cpc__profile_menu">Aktivität</div>';
				}
				echo '<div id="menu_activity" class="__cpc__profile_menu">Freunde-Aktivität</div>';
				echo '<div id="menu_all" class="__cpc__profile_menu">Alle Aktivitäten</div>';
			...
			echo '<div style="clear:both;">';
				echo $cpc_ui->profile_placeholder("wall", "__cpc__placeholder");
			echo '</div>';
